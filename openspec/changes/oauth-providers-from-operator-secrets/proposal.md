...
5. End-to-end integration tests against a `wiremock` mock IdP via the `nebula-api` `test_support` module gated by **custom cfg `nebula_test_util`** (D-14, wave-5 revision — NOT a Cargo feature, to avoid transitive activation; opt-in via `RUSTFLAGS="--cfg nebula_test_util"`). The module exposes bypass helpers for all three server-side IdP HTTP call sites (token POST + userinfo GET + OIDC discovery GET). Tests cover: happy-path session mint, replay rejection, state expiry, mismatched-provider, `public_url` change mid-flow, IdP token-endpoint failure, first-login user creation, existing-user linking by verified-email match, account-takeover defense (unverified Nebula email rejects OAuth link), provider-not-configured.
...
| 2 | **Trait + config + compose validation + test_support module** | `AuthBackend::start_oauth(provider, redirect_uri)` AND `complete_oauth(provider, state, code, redirect_uri)` signature changes (redirect_uri **auto-derived** from `ApiConfig::public_url` by the handler for both endpoints via shared helper, NOT user-supplied); `OAuthProvidersConfig` types per recon-4 §4 (Oidc vs Manual tagged union); env binding helpers; compose-root validation; `crates/api/src/test_support.rs` gated by `#[cfg(nebula_test_util)]` (custom cfg, NOT Cargo feature — wave-5 revision) exposing bypass helpers for token POST + userinfo GET + OIDC discovery GET; release-build guard via `cfg(all(nebula_test_util, not(debug_assertions))) compile_error!`; CI negative-test job. | ~360 | api + apps/server CODEOWNERS. |
