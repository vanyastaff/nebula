...
| 2 (trait + config + compose validation + test-util) | `auth_handler_derives_redirect_uri_from_public_url_for_both_start_and_complete`, `compose_root_fails_closed_when_public_url_unset_with_oauth_declared`, `oauth_provider_config_validates_oidc_requires_https_discovery_url`, `oauth_provider_config_validates_manual_requires_non_empty_scopes`, `oauth_provider_config_rejects_http_endpoint_in_release_mode` | trait sig extension on BOTH `start_oauth(provider, redirect_uri)` AND `complete_oauth(provider, state, code, redirect_uri)` + handler derives redirect_uri from `ApiConfig::public_url` for both endpoints + config types + env binding + compose-root validation + `test-util` feature exposing `flow::exchange_code_unchecked` | env-binding round-trip; Manual missing scopes rejected; provider key typo (`gooogle`) fails at config-load | inline cleanup |
