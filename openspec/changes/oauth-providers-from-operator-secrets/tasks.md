...
| T2.7 | RED | `auth_handler_extracts_derived_redirect_uri_from_public_url` (handler-level test that verifies the redirect_uri argument to `backend.start_oauth` matches `format!("{}/auth/oauth/{}/callback", public_url, provider)`). | ~40 |
...
| T2.9 | GREEN | Extend `AuthBackend` trait at `crates/api/src/domain/auth/backend/provider.rs`: `async fn start_oauth(&self, provider, redirect_uri: &str) -> Result<OAuthStart, AuthError>`. Update both implementors (`PgAuthBackend`, `InMemoryAuthBackend`) to accept and pass through the new arg (still build the synthetic URL — no functional change in this PR). Update every mock in `crates/api/tests/*.rs`. | ~60 |
| T2.10 | GREEN | Update handler in `crates/api/src/domain/auth/handler.rs::oauth_start` to derive `redirect_uri = format!("{}/auth/oauth/{}/callback", state.api_config.public_url, provider.as_str())` and pass to backend. | ~25 |
| T2.11 | SCAFFOLD | Add `test-util` feature to `crates/api/Cargo.toml`. Behind `#[cfg(any(test, feature = "test-util"))]`, re-export `flow::exchange_code_unchecked` as `pub` in `crates/api/src/transport/oauth/mod.rs`. | ~20 |
| T2.12 | GREEN | Add a `release` feature build-time assertion in `apps/server/src/main.rs`: `#[cfg(all(feature = "release", feature = "nebula-api/test-util"))] compile_error!("test-util must not be enabled in release builds")`. Or equivalent CI gate in `lefthook.yml`. | ~15 |
...
**RED test count for PR-2**: 5 (T2.3, T2.4, T2.5, T2.6, T2.7).
...
| # | Tag | Description | LOC |
|---|---|---|---|
| T4.1 | SCAFFOLD | Create migration `crates/storage/migrations/postgres/00XX_external_identities.sql` (number TBD per migration sequence at PR time). Table per D-8: `(user_id UUID REFERENCES users(id) ON DELETE CASCADE, provider TEXT, subject TEXT, email TEXT, linked_at TIMESTAMPTZ, PRIMARY KEY (provider, subject))` + `external_identities_user_id_idx`. | ~30 |
...
| T4.17 | GREEN | Rewrite `PgAuthBackend::complete_oauth` at `pg.rs:1079-1110`. Steps per spec REQ-oauth-003 (RECON-4 revised): consume_by_state_and_provider, verify redirect_uri matches derived (REQ-oauth-003 step 4 + Scenario 3.10), build `TokenExchangeRequest` from provider config + row.code_verifier, call `flow::exchange_code(req)`, log `id_token` presence (no validation per D-16), GET userinfo via `oauth_token_http_client()` with `Authorization: Bearer <access_token>`, parse `email` + `sub` + `email_verified`, find-or-create user via `external_identities` + truth table (REQ-oauth-005), mint session via existing session pipeline, drop tokens. **Remove the `Err(AuthError::NotImplemented(...))` line.** | ~140 |
