...
| T2.5 | RED | `oauth_provider_config_rejects_http_endpoint_for_all_server_side_urls` — verifies `validate_oauth_outbound_url` (renamed from `validate_token_endpoint` per D-9-WAVE6) is invoked on `Oidc.discovery_url`, `Manual.token_url`, `Manual.userinfo_url`, `Manual.verified_emails_url` (when Some), and `Manual.jwks_url` (when Some). HTTP scheme + localhost/private/loopback IPs all rejected uniformly. | ~50 |
...
| T4.17 | GREEN | Rewrite `PgAuthBackend::complete_oauth` at `pg.rs:1079-1110`. Steps per spec REQ-oauth-003 (recon-3/4 + wave-6 revised): `oauth_state_repo.consume_by_state_and_provider(state, provider.as_str())` (Plane A `OAuthStateRepo`); verify the row's `redirect_uri` equals the handler-supplied arg — if not, `AuthError::OAuthFailed { cause: "public_url_changed_mid_flow" }` (Scenario 3.10); build `flow::TokenExchangeRequest` from provider config + row.code_verifier + code + redirect_uri; call `flow::exchange_code(req)` (per D-12); **log `id_token` field presence only — NO JWKS signature validation per D-16 defer**; resolve `userinfo_url` (Oidc → cached `OidcDiscovery.userinfo_url`, already validated per D-15-WAVE6; Manual → `provider_config.endpoints.userinfo_url`, validated at boot per Invariant 1); call `validate_oauth_outbound_url(userinfo_url)` defensively before the GET per D-9-WAVE6; GET userinfo via `oauth_token_http_client()` with `Authorization: Bearer <access_token>`; parse `sub` + `email`; **if `email_verified` claim is absent AND `provider_config.endpoints` is `Manual { verified_emails_url: Some(url), .. }`**: `validate_oauth_outbound_url(url)` + GET it with the same Bearer header, parse `[{email, primary, verified}]`, pick the entry where `primary && verified` as the canonical `(email, email_verified=true)` pair; otherwise treat absence as `email_verified=false` (REQ-oauth-005 truth-table input); find-or-create user via `external_identities` + truth table; mint session; drop tokens. **Remove the `Err(AuthError::NotImplemented(...))` line.** | ~180 |
...
**RED test count for PR-4**: 15 (T4.4 through T4.16, plus wave-6 additions T4.13b + T4.16b).
