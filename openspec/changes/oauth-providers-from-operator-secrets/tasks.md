...
| # | Tag | Description | LOC |
|---|---|---|---|
| T2.1 | SCAFFOLD | Create `crates/api/src/config/oauth.rs` (new file). Define `OAuthProvidersConfig` (outer map), `OAuthProviderConfig` (per-provider), `OAuthEndpoints` enum: `Oidc { discovery_url }` vs `Manual { authorize_url, token_url, userinfo_url, verified_emails_url: Option<String>, jwks_url: Option<String>, scopes: Vec<String> }` — `verified_emails_url` is wave-6 P2 addition for providers like GitHub whose `/user` doesn't return `email_verified` (PR-4 hits this second endpoint per REQ-oauth-001 Invariant 1). Derive Serialize/Deserialize/Debug. `client_id` and `client_secret` are `SecretString`. | ~130 |
...
| T2.5 | RED | Three RED tests for the two-validator split (🟥 WAVE-7 per Codex F.2): (a) `oauth_provider_config_rejects_http_endpoint_for_all_server_side_urls` — STRICT `validate_oauth_outbound_url` is invoked on `Oidc.discovery_url`, `Manual.token_url`, `Manual.userinfo_url`, `Manual.verified_emails_url`/`Manual.jwks_url` (when Some); HTTP + localhost rejected. (b) `oauth_provider_config_authorize_url_uses_flag_aware_validator` — `Manual.authorize_url` goes through FLAG-AWARE `validate_oauth_authorize_url` which accepts `http://localhost(:port)?` when `oauth_allow_insecure_localhost = true` AND `cfg!(debug_assertions)`. (c) `oauth_authorize_url_strict_in_release_build` — flag has no effect when binary is built in release profile (`!cfg!(debug_assertions)`). | ~80 |
...
| T2.7b | SCAFFOLD | (🟥 WAVE-7) Add a public helper `validate_oauth_authorize_url(url: &str, flag: bool, in_release: bool) -> Result<(), TokenHttpError>` in `crates/api/src/transport/oauth/flow.rs` that wraps `validate_oauth_outbound_url` but allows `http://localhost(:port)?` when `flag && !in_release`. Header comment: "this gate is ONLY for `authorize_url` (browser-fetched) — server-side fetch sites use the strict gate per D-9-WAVE6". | ~40 |
...
| T4.4 | RED | `complete_oauth_succeeds_with_valid_code_oidc_provider_first_login` (full happy path via wiremock; REQ-oauth-004 first-login + creates external_identities row). | ~70 |
| T4.4b | RED | (🟥 WAVE-7) `complete_oauth_short_circuits_via_external_identity_on_repeated_login` (REQ-oauth-006 Scenario 6.1: user logs in second time, `find_user_by_external` returns Some, session minted directly without consulting email truth-table; no duplicate user row; no new external_identities row). | ~60 |
| T4.4c | RED | (🟥 WAVE-7) `complete_oauth_external_identity_link_survives_idp_email_change` (REQ-oauth-006 Scenario 6.2: IdP email changed since first link; session still minted via stable `sub`; external_identities.email snapshot NOT updated). | ~50 |
...
| T4.13 | RED | `complete_oauth_creates_user_on_first_login_with_idp_verified_email_oidc_inline` (OIDC provider — `email_verified=true` in userinfo response; REQ-oauth-004). | ~55 |
| T4.13b | RED | `complete_oauth_creates_user_on_first_login_with_github_verified_email_via_second_endpoint` (🟥 WAVE-6: GitHub provider — `/user` has no `email_verified`; `/user/emails` returns `[{primary:true, verified:true}]`; user is created). | ~60 |
| T4.14 | RED | `complete_oauth_rejects_first_login_when_idp_email_unverified` (REQ-oauth-004; covers both inline `email_verified=false` AND github-style `verified_emails_url` returning no primary-verified entry). | ~50 |
...
| T4.16b | RED | `complete_oauth_rejects_github_userinfo_when_verified_emails_url_returns_internal_ip` (🟥 WAVE-6 SSRF defense: if a hostile operator config or compromised IdP somehow gets `verified_emails_url = "http://10.0.0.5/admin"` past boot validation, the defensive `validate_oauth_outbound_url` call inside `complete_oauth` MUST still reject it). | ~30 |
| T4.17 | GREEN | Rewrite `PgAuthBackend::complete_oauth` at `pg.rs:1079-1110` per REQ-oauth-003 (recon-3/4 + wave-6/-7 revised): `oauth_state_repo.consume_by_state_and_provider(...)`; verify row's `redirect_uri` == handler-supplied arg; build `flow::TokenExchangeRequest`; `flow::exchange_code(req)`; log `id_token` presence only (D-16 defer); resolve `userinfo_url`; `validate_oauth_outbound_url(userinfo_url)` defensively (D-9-WAVE6); GET userinfo with Bearer; parse `sub` + `email`; **if `email_verified` absent AND Manual `verified_emails_url: Some(url)`**: validate + GET, pick `primary && verified` entry; **REQ-oauth-006 short-circuit (🟥 WAVE-7)**: `external_identity_repo.find_user_by_external(provider, sub)` — if Some(user_id), mint session for that user_id and return (no truth-table consultation); else apply REQ-oauth-004 / REQ-oauth-005 truth tables; on first-login or new link, `external_identity_repo.link_external(user_id, provider, sub, email)`; mint session; drop tokens. **Remove `Err(AuthError::NotImplemented(...))` line.** | ~200 |
...
**RED test count for PR-4**: 17. T4.4-T4.16 baseline (13) + wave-6 additions T4.13b/T4.16b (+2) + wave-7 additions T4.4b/T4.4c (+2 for REQ-oauth-006 already-linked path).
