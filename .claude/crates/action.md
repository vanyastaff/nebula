...
- `ActionHandler` enum (5 variants: Stateless, Stateful, Trigger, Resource, Agent) — engine match-dispatches. Replaces deprecated `InternalHandler`.
...
- `StatefulAction::State` now requires `Serialize + DeserializeOwned + Clone`. `init_state()` is required.
...
- `ResourceActionAdapter::cleanup` downcasts `Box<dyn Any>` — panics on type mismatch (returns Fatal error, not panic).
