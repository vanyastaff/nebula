...
- `stateful.rs` — `StatefulAction` core + DX (`PaginatedAction`, `BatchAction`) + `macro_rules!` macros (`impl_paginated_action!`, `impl_batch_action!`). Macros generate `impl StatefulAction for $ty` — no blanket impls (Rust coherence forbids multiple). Engine never sees DX types. **`TransactionalAction` and `impl_transactional_action!` were deleted 2026-04-10** (M1) — the three-phase state machine was unreachable past the first phase in actual engine dispatch; saga orchestration will ship with its own trait shape post-v1.
...
- `PollTriggerAdapter` clamps `poll_interval()` to `POLL_INTERVAL_FLOOR = Duration::from_millis(100)`. Shorter intervals are a busy-loop bug or an abusive integration; legitimate high-frequency event sources should use streaming triggers (post-v1). The clamp logs a `tracing::warn!` so the divergence from the action's declared interval is visible in ops. Tests that need to exercise the adapter must use ≥100 ms intervals to avoid fighting the clamp. Silent error drops in the poll loop (retryable poll error, event serialization failure, emit failure) now log via `tracing` — this is a temporary direct `tracing` usage; Phase 5 / PR 8 will migrate to `ctx.logger` with `WarnThrottle`.
