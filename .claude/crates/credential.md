...
- v2 `PostgresStore` (`store_postgres.rs`): KV-based `CredentialStore` impl behind `storage-postgres`. Wraps `nebula_storage::Storage<Key=String, Value=Vec<u8>>`. Keys prefixed `cred:{id}`. Uses `StoredEntry` serde wrapper (base64 `data`). CAS is optimistic read-then-write (not truly atomic — KV trait has no CAS primitive). `list()` returns empty (Storage trait has no prefix iteration).
- v2 `VaultStore` (`store_vault.rs`): HashiCorp Vault KV v2 `CredentialStore` impl behind `storage-vault`. Token + AppRole auth, namespace support, background token renewal (placeholder). Uses `VaultPayload` serde wrapper (base64 data, RFC3339 timestamps, JSON-string metadata). CAS via logical version check + read-then-write. `list()` uses `kv2::list` with `state_kind` filtering via individual reads.
