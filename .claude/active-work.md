...
## Recently Completed
- **Telemetry strip to pure metrics** (04-04): Removed ExecutionEvent, EventBus wrapper, TelemetryService trait, TraceContext from nebula-telemetry. Removed EventBus from engine and runtime (metrics only). Dependencies reduced from 11 to 5. Events will be redesigned when engine stabilizes.
