...
- **[L2-§11.3]** Remote effects are not atomic with Nebula's database. The current
  ...
  acknowledges a fresh call grant. After an ambiguous boundary, only a pinned stable-key destination may make
  ...
  `OutcomeUnknown`, after which no effecting re-invocation is allowed. The runtime
  ...
  permits only ledger reads and exact frozen-evidence recommit. An outstanding call
  found after a crash becomes unknown and may only use the separate
