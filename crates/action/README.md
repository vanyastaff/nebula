...
**Action Trait Family + Execution Policy Metadata (Ports & Adapters pattern)**. Core types and traits live here; the engine dispatches actions **in-process** (`InProcessRunner`). Process/WASM isolation is a non-goal (ADR-0091, `docs/PRODUCT_CANON.md` §12.6).
...
- `IsolationLevel`, `ActionCategory`, `ActionKind`, `CheckpointPolicy` — in-process capability gating, UI grouping, node-taxonomy classification, and checkpoint cadence.
...
- Not the execution driver — the engine runs actions in-process (`InProcessRunner`); process/WASM isolation is a non-goal (ADR-0091).
