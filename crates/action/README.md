...
- `WebhookAction` — webhook lifecycle with HMAC verification (DX over `TriggerAction`). Returns `WebhookConfig` from `config()` so the HTTP transport can enforce the signature policy *before* calling `handle_request`.
- `WebhookConfig`, `SignaturePolicy`, `RequiredPolicy`, `SignatureScheme` — ADR-0022 signature enforcement surface. Default policy is `Required` with an empty secret (fail-closed); explicit `OptionalAcceptUnsigned` is the opt-out audit trail; `Custom(fn)` covers Stripe / Slack / bespoke schemes by composing the primitives in this module.
