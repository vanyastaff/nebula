...
| Plane-A OAuth discovery / bounded token exchange / userinfo | `transport/oauth/{discovery,flow,http,userinfo}.rs` |
...
- `transport/` — `credential` (service-layer через фасад), `oauth`
  (Plane-A-only discovery/flow/http/userinfo), `webhook` (единый inbound:
...
- **Durability-фронтир.** Credential CRUD уже идёт через файловый SQLite
  ...
  process-local. `me/*` и org-membership работают end-to-end, но для них
  wired backend по-прежнему in-memory (нет storage-backed
  `AuthBackend`/`UserRepo`/`PatRepo`/`SessionRepo`/membership repo). 1.0-closure
  требует storage-backed auth/membership; multi-replica credentials дополнительно
  требуют shared backend и durable pending-store.
