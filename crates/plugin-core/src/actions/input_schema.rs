...
use nebula_schema::{
    DynamicField, FieldKey, ListField, ObjectField, Predicate, Property, Rule, ValidationError,
...
pub(super) fn strings(key: FieldKey) -> ListField {
    Property::list(key).item(Property::string(field_key!("item")))
}
...
pub(super) fn nullable_object_data() -> DynamicField {
    // Property::object is non-nullable even when optional. Keep null and arbitrary
    // keys intact; the action's existing object-or-null check remains authoritative.
    Property::dynamic(field_key!("data"))
