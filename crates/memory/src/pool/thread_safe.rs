...
impl<T: Poolable> ThreadSafePooledValue<'_, T> {
    ...
    pub fn detach(mut self) -> T {
        ...
        // - mem::forget prevents Drop::drop from running, so we don't return to pool
        // - We take full ownership of the value
...
impl<T: Poolable> Drop for ThreadSafePooledValue<'_, T> {
    fn drop(&mut self) {
        // SAFETY: Taking value out of ManuallyDrop to return to pool.
        // - value is initialized
        // - We are in Drop, so value won't be used again
        // - pool reference is valid for 'a (lifetime of this struct)
