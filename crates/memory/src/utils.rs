...
pub unsafe fn copy_aligned_simd(dst: *mut u8, src: *const u8, len: usize) {
    // SAFETY: Delegate to copy_nonoverlapping
    // - Caller guarantees validity of src, dst and no overlap
...
pub unsafe fn copy_aligned_simd(dst: *mut u8, src: *const u8, len: usize) {
    // SAFETY: Caller ensures dst and src are valid for len bytes and do not overlap
    unsafe {
...
impl Timer {
    ...
    pub fn print(&self) {
        #[cfg(feature = "logging")]
        nebula_log::info!("{}: {}", self.name, format_duration(self.elapsed()));
    }
}
