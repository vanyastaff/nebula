//! Property definitions — consolidated enum (13 variants) + per-type structs + builders.
//!
...
/// Generate a per-type property struct with shared base fields + type-specific extras.
///
...
/// before converting to `Property`.
macro_rules! define_field {
    (@definition $name:ident { $($(#[$attribute:meta])* pub $field:ident: $ty:ty,)* }) => {
        ...
        impl<'de> Deserialize<'de> for $name {
            ...
            fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
                ...
                // The same slots define the wire and public type; the boundary hides serde payloads.
                #[derive(Deserialize)]
                struct Definition {
                    $($(#[$attribute])* $field: $ty,)*
                }
                let definition = Definition::deserialize(deserializer).map_err(|error| {
                    D::Error::custom(redacted_known_field_error(stringify!($name), &error.to_string()))
                })?;
                Ok(Self { $($field: definition.$field,)* })
            }
        }
    };
    (@debug SecretField { $($extra:ident),* }) => {
        ...
        impl std::fmt::Debug for $name {
            fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter
                    ...
                    // Ancestor defaults can contain protected descendants, even before lint.
                    .field("default", &self.default.as_ref().map(|_| "<redacted>"))
                    .field("visible", &self.visible)
                    .field("required", &self.required)
                    .field("expression", &self.expression)
                    .field("group", &self.group)
                    .field("read_aliases", &self.read_aliases)
                    .field("emit_as", &self.emit_as)
    ...
    ($name:ident { $($extra:ident: $ty:ty = $dflt:expr),* $(,)? } expr: $expr_dflt:expr) => {
        ...
        impl $name {
            ...
            /// Finalize this typed declaration as a semantic property.
            #[must_use]
...
/// Enum wrapper for typed semantic schema properties (13 variants).
///
/// Removed in this version: `Date`, `DateTime`, `Time`, `Color`, `Hidden`.
/// - Date-like: use `Property::string(k).hint(InputHint::Date)` etc.
/// - Hidden: use `.visible(VisibilityMode::Never)` on any property.
#[non_exhaustive]
...
pub enum Property {
    ...
    /// round-trips (object key order is normalized by the wire backend, not
    /// retained), and the shared fields (`key`/`visible`/`required`) are
    /// recovered as descriptive metadata. Current admission, export and
    /// schema-bound value projections reject unknown declarations; compatibility
    /// analysis cannot prove their rules or assignability. Debug output is redacted
    /// because the protection policy of the raw descriptor is unknown.
...
impl std::fmt::Debug for UnknownField {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str("UnknownField(<redacted>)")
    }
}
...
impl From<KnownField> for Property {
...
impl<'de> Deserialize<'de> for Property {
    ...
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        // Distinguish an absent `type` from a present-but-non-string one so the
        // diagnostic is accurate (a number `type` is not a "missing field").
        let type_tag = match value.get("type") {
            None => return Err(D::Error::missing_field("type")),
            Some(Value::String(tag)) => tag.as_str(),
            Some(_) => return Err(D::Error::custom("field `type` must be a string")),
        };
        if KNOWN_FIELD_TYPES.contains(&type_tag) {
            ...
            return KnownField::deserialize(&value)
                .map(Property::from)
                .map_err(|error| {
                    // Only whitelisted kind names reach this branch; serde causes may hold secrets.
                    D::Error::custom(redacted_known_field_error(type_tag, &error.to_string()))
                });
        }
        // Forward compatibility: an unrecognized `type` is preserved (every key
        // and value kept in `raw`), with the shared fields recovered so the
        // editor can still place it.
        let unknown_type = type_tag.to_owned();
        let key = value
            .get("key")
            .and_then(Value::as_str)
            .and_then(|raw_key| FieldKey::new(raw_key).ok())
            .ok_or_else(|| D::Error::custom("unknown field is missing a valid `key`"))?;
        ...
        // rather than erroring: the true value is still preserved in `raw`, and
        // refusing to default would defeat the whole point of `Unknown` (never
        // failing to read a document a newer writer produced).
        let visible = value
            .get("visible")
            .cloned()
        ...
        Ok(Property::Unknown(UnknownField {
            unknown_type,
            key,
            visible,
            required,
...
fn redacted_known_field_error(type_tag: &str, detail: &str) -> String {
    // Serde erases typed causes. Only exact, payload-free owner diagnostics survive;
    // a code mentioned inside a malformed widget or key is not a trusted diagnostic.
    let diagnostic = match detail {
        "[transformer.invalid_pattern]: regex transformer pattern could not be compiled" => {
            "[transformer.invalid_pattern]: regex transformer pattern could not be compiled"
        },
        "[transformer.invalid_capture_group]: regex transformer capture group does not exist" => {
            "[transformer.invalid_capture_group]: regex transformer capture group does not exist"
        },
        _ => return format!("invalid {type_tag} field descriptor"),
    };
    diagnostic.to_owned()
}
...
impl Property {
    ...
    pub fn unknown_type(&self) -> Option<&str> {
        match self {
            Self::Unknown(f) => Some(&f.unknown_type),
            _ => None,
        }
    }
    ...
    #[inline]
    #[must_use]
    pub fn raw_object(&self) -> Option<&serde_json::Map<String, Value>> {
        match self {
            Self::Unknown(f) => Some(&f.raw),
            _ => None,
        }
    }
}
...
impl From<NoticeField> for Property {
