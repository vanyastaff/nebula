...
fn no_payload_variant_string_marker_payload_is_rejected_at_validate() {
    ...
    // object) submitted as the variant payload. Explicit authoring ingestion turns
...
fn hidden_no_payload_variant_literal_payload_still_builds_and_resolves() {
    ...
    let values = AuthoredValue::from_template_json(json!({ "auth": { "mode": "none" } }))
        .expect("from_json");
    ...
    assert_eq!(
        prepared.resolve_data().unwrap().into_json(),
        json!({"auth": {"mode": "none"}})
    );
}
