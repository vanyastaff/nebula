...
- `src/decorator/mod.rs` + `decorator/*.rs` — `Scoped*Store` wrappers for execution, workflow, control_queue, idempotency, journal, node_result, shared-resource subscription/fanout/handoff, resource, trigger, and webhook; each substitutes the bound scope on every call.
...
- Decorators do not own an audit or tracing sink. Cross-tenant denial observations belong at the authenticated coordinator boundary; do not infer a denial from a substituted caller scope or add a second audit path here.
