...
proptest! {
    ...
    fn array_length_matches_vec(items in prop::collection::vec(any::<i32>(), 0..100)) {
        let value_items: Vec<_> = items.iter().map(|&i| Value::integer(i as i64)).collect();
        let array = Array::from_vec(value_items);
    ...
    fn array_get_in_bounds(items in prop::collection::vec(any::<i32>(), 1..100), idx in 0usize..99) {
        let value_items: Vec<_> = items.iter().map(|&i| Value::integer(i as i64)).collect();
        let array = Array::from_vec(value_items.clone());

        if idx < items.len() {
            let value = array.get(idx);
            prop_assert!(value.is_some());
            prop_assert_eq!(value, Some(&value_items[idx]));
        }
    }
    ...
    fn array_push_increases_length(items in prop::collection::vec(any::<i32>(), 0..100), new_val in any::<i32>()) {
        ...
        let new_val_wrapped = Value::integer(new_val as i64);
        let new_array = array.push(new_val_wrapped.clone());

        prop_assert_eq!(new_array.len(), original_len + 1);
        prop_assert_eq!(new_array.get(original_len), Some(&new_val_wrapped));
    }
    ...
    fn array_concat_length_sum(a in prop::collection::vec(any::<i32>(), 0..50), b in prop::collection::vec(any::<i32>(), 0..50)) {
        ...
        let arr_a = Array::from_vec(values_a);
        let arr_b = Array::from_vec(values_b);
    ...
    fn array_clone_equals_original(items in prop::collection::vec(any::<i32>(), 0..100)) {
        ...
        for (i, _item) in value_items.iter().enumerate() {
    ...
    fn array_push_original_unchanged(items in prop::collection::vec(any::<i32>(), 1..50), new_val in any::<i32>()) {
        let value_items: Vec<_> = items.iter().map(|&i| Value::integer(i as i64)).collect();
        let original = Array::from_vec(value_items.clone());
        let original_len = original.len();

        let _modified = original.push(Value::integer(new_val as i64));

        // Original should be unchanged
        prop_assert_eq!(original.len(), original_len);
        for (i, item) in value_items.iter().enumerate() {
...
proptest! {
    ...
    fn object_insert_preserves_value(key in ".*", val in any::<i32>()) {
        ...
        let val_wrapped = Value::integer(val as i64);
        let new_object = object.insert(key.clone(), val_wrapped.clone());

        prop_assert_eq!(new_object.get(&key), Some(&val_wrapped));
    }
    ...
    fn object_clone_equals_original(entries in prop::collection::vec((".*", any::<i32>()), 0..20)) {
        ...
        for (key, _val) in json_entries.iter() {
    ...
    fn object_insert_original_unchanged(key in ".*", val in any::<i32>()) {
        ...
        let _modified = original.insert(key.clone(), Value::integer(val as i64));
