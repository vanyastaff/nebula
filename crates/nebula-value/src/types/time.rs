...
/// Rich, typed errors for Time operations
#[derive(Error, Debug, Clone, PartialEq, Eq)]
pub enum TimeError {
    #[error("Invalid time: hour={hour}, minute={minute}, second={second}")]
    InvalidTime { hour: u32, minute: u32, second: u32 },

    #[error("Time out of range: {msg}")]
    OutOfRange { msg: String },

    #[error("Invalid component: {component} = {value}")]
    InvalidComponent { component: &'static str, value: u64 },
    ...
    #[error("Arithmetic overflow in time operation")]
    ArithmeticOverflow,

    #[error("Duration too large for time operation")]
    DurationTooLarge,
}
...
impl TimeInner {
    ...
    pub fn from_nanos(nanos: u64) -> TimeResult<Self> {
        if nanos > Self::MAX_NANOS {
            return Err(TimeError::OutOfRange {
                msg: format!("Nanoseconds {} exceeds max {}", nanos, Self::MAX_NANOS),
            });
        }
        Ok(Self { nanos })
    }
...
pub struct Time {
    ...
    iso_string_cache: std::sync::OnceLock<String>,

    /// Cached 12-hour format string
    #[cfg_attr(feature = "serde", serde(skip))]
    format_12h_cache: std::sync::OnceLock<String>,
}
...
impl Time {
    ...
    /// Creates a new Time
    pub fn new(hour: u32, minute: u32, second: u32) -> TimeResult<Self> {
        Self::with_nanos(hour, minute, second, 0)
    }

    /// Creates a new Time with nanoseconds
    pub fn with_nanos(hour: u32, minute: u32, second: u32, nanos: u32) -> TimeResult<Self> {
        let inner = TimeInner::new(hour, minute, second, nanos)?;
        Ok(Self {
            inner: Arc::new(inner),
            iso_string_cache: std::sync::OnceLock::new(),
            format_12h_cache: std::sync::OnceLock::new(),
        })
    }

    /// Creates a new Time with milliseconds
    pub fn with_millis(hour: u32, minute: u32, second: u32, millis: u32) -> TimeResult<Self> {
        if millis >= 1000 {
            return Err(TimeError::InvalidComponent {
    ...
    pub fn with_micros(hour: u32, minute: u32, second: u32, micros: u32) -> TimeResult<Self> {
        if micros >= 1_000_000 {
            return Err(TimeError::InvalidComponent {
    ...
    pub fn from_millis(millis: u64) -> TimeResult<Self> {
        if millis >= 86_400_000 {
            return Err(TimeError::OutOfRange {
                msg: format!("Milliseconds {} >= 86400000", millis),
            });
        }
    ...
    pub fn parse_iso(s: &str) -> TimeResult<Self> {
        ...
        let (time_part, nano_part) = if let Some(dot_pos) = s.find('.') {
            ...
            let nanos = if nano_str.len() <= 9 {
                ...
                padded.parse::<u32>().map_err(|_| TimeError::ParseError {
                    msg: format!("Invalid nanoseconds: {}", nano_str),
                })?
            } else {
                return Err(TimeError::ParseError {
                    msg: format!("Too many decimal places: {}", nano_str),
                });
            };
            (&s[..dot_pos], nanos)
        } else {
        ...
        let hour = parts[0].parse::<u32>().map_err(|_| TimeError::ParseError {
            msg: format!("Invalid hour: {}", parts[0]),
        })?;

        let minute = parts[1].parse::<u32>().map_err(|_| TimeError::ParseError {
    ...
    pub fn parse_12h(s: &str) -> TimeResult<Self> {
        let s = s.trim();
        ...
        let hour12 = parts[0].parse::<u32>().map_err(|_| TimeError::ParseError {
            msg: format!("Invalid hour: {}", parts[0]),
        })?;

        if hour12 == 0 || hour12 > 12 {
            return Err(TimeError::InvalidComponent {
                component: "hour (12-hour format)",
                value: hour12 as u64,
            });
        }
    ...
    pub fn add_duration(&self, duration: StdDuration) -> TimeResult<Self> {
    ...
    pub fn sub_duration(&self, duration: StdDuration) -> TimeResult<Self> {
        let total_nanos = duration.as_nanos();
        if total_nanos > TimeInner::MAX_NANOS as u128 {
            return Err(TimeError::DurationTooLarge);
        }
