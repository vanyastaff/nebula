...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = timeout(Duration::from_millis(100), async { "operation completed" }).await;
    println!("   Result: {:?}\n", result);

    // Example 2: Retry with exponential backoff
    println!("2. Retry Strategy Example");
    let retry_strategy = retry::RetryStrategy::new(3, Duration::from_millis(10)).with_jitter(0.1);
