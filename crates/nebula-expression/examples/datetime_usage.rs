...
fn main() {
    ...
    let result = engine
        .evaluate("{{ now() | format_date(\"DD.MM.YYYY HH:mm:ss\") }}", &context)
        .unwrap();
    ...
    println!("\nExample 4: Date arithmetic");
    let result = engine
        .evaluate("{{ now() | date_add(7, \"days\") | format_date(\"YYYY-MM-DD\") }}", &context)
        .unwrap();
    println!("  7 days from now: {}", result);

    let result = engine
        .evaluate("{{ now() | date_subtract(30, \"days\") | format_date(\"YYYY-MM-DD\") }}", &context)
        .unwrap();
    println!("  30 days ago: {}", result);

    let result = engine
        .evaluate("{{ now() | date_add(2, \"hours\") | format_date(\"HH:mm\") }}", &context)
        .unwrap();
    ...
    let result = engine.evaluate("{{ parse_date($input) | date_year() }}", &context).unwrap();
    println!("  Year: {}", result);

    let result = engine.evaluate("{{ parse_date($input) | date_month() }}", &context).unwrap();
    println!("  Month: {}", result);

    let result = engine.evaluate("{{ parse_date($input) | date_day() }}", &context).unwrap();
    println!("  Day: {}", result);

    let result = engine.evaluate("{{ parse_date($input) | date_hour() }}", &context).unwrap();
    println!("  Hour: {}", result);

    let result = engine.evaluate("{{ parse_date($input) | date_minute() }}", &context).unwrap();
    println!("  Minute: {}", result);

    let result = engine.evaluate("{{ parse_date($input) | date_second() }}", &context).unwrap();
    println!("  Second: {}", result);

    // Example 7: Day of week
    println!("\nExample 7: Day of week");
    let result = engine
        .evaluate("{{ parse_date(\"2024-06-15\") | date_day_of_week() }}", &context)
        .unwrap();
