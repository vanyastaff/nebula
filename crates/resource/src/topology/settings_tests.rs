...
fn pool_settings_map_every_field() {
    let config = pool(json!({
        ...
        "strategy": "fifo",
        "warmup": { "mode": "staggered", "interval_ms": 250 },
...
fn unknown_or_mistyped_fields_are_rejected() {
    ...
    assert!(pool(json!({ "warmup": { "mode": "eager" } })).is_err());
}
...
fn bounded_settings_require_an_explicit_mode() {
    assert_eq!(
        serde_json::from_value::<BoundedSettings>(json!({ "mode": "capped", "max_concurrent": 3 }))
            .expect("valid"),
        BoundedSettings::Capped { max_concurrent: 3 }
    );
    assert!(serde_json::from_value::<BoundedSettings>(json!({})).is_err());
    assert!(
        serde_json::from_value::<BoundedSettings>(json!({ "mode": "capped" })).is_err(),
        "capped needs its cap"
    );
}
