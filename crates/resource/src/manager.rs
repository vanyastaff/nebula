...
impl Manager {
    ...
    /// # Errors
    ///
    /// Returns [`ErrorKind::Permanent`] if a resource with the same key
    /// and scope is already registered.
    pub fn register<R: Resource>(
        ...
        let managed = Arc::new(ManagedResource {
            ...
            status: arc_swap::ArcSwap::from_pointee(ResourceStatus::new()),
        });

        let type_id = TypeId::of::<ManagedResource<R>>();
        self.registry
            .register(key.clone(), type_id, scope, managed);
    ...
    /// - [`ErrorKind::NotFound`] if no resource of type `R` is registered.
    /// - [`ErrorKind::Cancelled`] if the manager is shutting down.
    /// - [`ErrorKind::Permanent`] if the resource is not using pool topology.
    ...
    ///
    /// - [`ErrorKind::NotFound`] if no resource of type `R` is registered.
    /// - [`ErrorKind::Permanent`] if the resource is not using resident topology.
        ...
        R::Lease: Clone + Send + 'static,
    {
        ...
        let result = match &managed.topology {
            TopologyRuntime::Resident(rt) => {
                rt.acquire(&managed.resource, &config, credential, ctx).await
            }
            _ => Err(Error::permanent(format!(
    ...
    ///
    /// - [`ErrorKind::NotFound`] if no resource of type `R` is registered.
    /// - [`ErrorKind::Permanent`] if the resource is not using service topology.
    ...
    ///
    /// - [`ErrorKind::NotFound`] if no resource of type `R` is registered.
    /// - [`ErrorKind::Permanent`] if the resource is not using transport topology.
    ...
    ///
    /// - [`ErrorKind::NotFound`] if no resource of type `R` is registered.
    /// - [`ErrorKind::Permanent`] if the resource is not using exclusive topology.
    ...
    /// # Errors
    ///
    /// Returns [`ErrorKind::NotFound`] if the key is not registered.
