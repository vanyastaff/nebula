...
fn guard_drop_calls_callback() {
    ...
    let guard = Guard::new(
        "guarded_resource".to_string(),
        move |_resource, _tainted| {
...
fn guard_deref_provides_access_to_inner() {
    let guard = Guard::new("hello".to_string(), |_, _| {});
...
fn guard_deref_mut_allows_modification() {
    let mut guard = Guard::new("hello".to_string(), |_, _| {});
...
fn guard_callback_receives_the_resource() {
    ...
    let guard = Guard::new("test_value".to_string(), move |resource, _tainted| {
