...
use nebula_resource::{
    Config, Context, Error, ExecutionId, Manager, ManagerBuilder, Pool, PoolAcquire, PoolConfig,
    PoolLifetime, PoolResiliencePolicy, PoolSizing, Resource, ResourceMetadata, Result, Scope,
    WorkflowId,
};
...
impl Resource for MortalResource {
    ...
    fn is_broken(&self, inst: &MortalInstance) -> nebula_resource::resource::BrokenCheck {
        if inst.born_at.elapsed() >= inst.ttl {
            nebula_resource::resource::BrokenCheck::Broken
        } else {
            nebula_resource::resource::BrokenCheck::Healthy
        }
    }
}
