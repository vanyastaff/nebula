...
| Rate-limit profile / interim `Limited` surface | `src/rate_limit_tests.rs` (profile latching, stable names), [rate_limit](tests/rate_limit.rs) (health snapshot and erased view per profile), README "Rate-limit profiles" table, SDK [public_api_snapshot](../sdk/tests/public_api_snapshot.rs) (`[interim]` tags; re-bless with `task sdk:api:bless`). Only `InterimPerClosure` is interim; `RateLimitProfile` stays out of the SDK. |
