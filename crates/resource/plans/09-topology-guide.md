...
> **Normative source:** `01-core.md`, `02-topology.md`, `08-correctness.md`.
> This guide is explanatory. If examples here conflict with normative docs, trust normative docs.
...
impl Resource for Postgres {
    ...
    type Error      = PgError;
    type Credential = DatabaseCredential; // framework resolves before create()
...
impl Resource for SmtpConnection {
    ...
    type Credential = SmtpCredential;     // { host, port, username, password }
    const KEY: ResourceKey = resource_key!("smtp");

    async fn create(
        &self,
        config: &SmtpResourceConfig,
        cred:   &SmtpCredential,
        _ctx:   &dyn Ctx,
...
impl Resident for HttpClient {
    // All defaults. reqwest::Client stateless — is_alive_sync always true, stale_after None.
}
```
...
impl Resource for RedisShared {
    ...
    type Credential = RedisCredential;    // { host, port, password }
    const KEY: ResourceKey = resource_key!("redis.shared");

    async fn create(
        &self,
        _config: &RedisResourceConfig,
        cred:    &RedisCredential,
        _ctx:    &dyn Ctx,
    ) -> Result<fred::Client, RedisError> {
...
impl Resident for RedisShared {
    fn is_alive_sync(&self, client: &fred::Client) -> bool {
        client.is_connected() // O(1) atomic flag, no I/O
    }
...
impl Resource for GrpcChannel {
    ...
    type Credential = GrpcCredential;             // { endpoint, tls_config }
    ...
    async fn create(
        ...
        cred:   &GrpcCredential,
        _ctx:   &dyn Ctx,
    ) -> Result<tonic::transport::Channel, GrpcError> {
...
- **❌ Resident без stale_after для network clients.** Без check — clone-ы молча мертвы после network failure. → Добавь `stale_after(Some(15s))` + `is_alive_sync()` (O(1), no I/O).
...
impl Resource for Ssh {
    ...
    type Error      = SshError;
    type Credential = SshKeyCredential;    // { username, private_key, passphrase }
...
impl Resource for KafkaConsumer {
    ...
    type Credential = KafkaCredential;    // { brokers, sasl_username, sasl_password }
    const KEY: ResourceKey = resource_key!("kafka.consumer");

    async fn create(
        &self,
        config: &KafkaConsumerResourceConfig,
        cred:   &KafkaCredential,
        _ctx:   &dyn Ctx,
...
impl Resource for RedisSubscriber {
    ...
    async fn create(
        ...
        cred:   &RedisCredential,
        _ctx:   &dyn Ctx,
    ) -> Result<RedisPubSubRuntime, RedisError> {
        let subscriber = fred::SubscriberClient::new(/* from cred.host, cred.port, cred.password */);
