...
//!
//! This is the non-negotiable §14 "no discard-and-log" rule.
...
/// (sub-spec `docs/INTEGRATION_MODEL.md` §6) and carry their structured
/// payload as enum fields. The same [`AuditSink`] receives both families
/// so operators reuse one sink implementation.
...
pub enum AuditOperation {
    ...
    /// `ReauthReason` published on the event bus. Sub-spec §6 audit event.
...
///
/// ```rust,ignore
/// use nebula_credential::store_layer::AuditLayer;
/// use std::sync::Arc;
///
...
impl<S: CredentialStore> CredentialStore for AuditLayer<S> {
    ...
    async fn put(
        ...
        if let Err(sink_err) = self.sink.record(&event) {
            // Fail-closed: best-effort rollback of the inner write so the
            // store ends in the pre-call state. Only attempted on CreateOnly
            // (Overwrite/CAS have no recoverable prior state at this layer).
...
mod tests {
    ...
    use super::{super::InMemoryCredentialStore, *};
    ...
    fn make_store(sink: &Arc<CollectingSink>) -> AuditLayer<InMemoryCredentialStore> {
        AuditLayer::new(
    ...
    async fn get_logs_audit_event() -> Result<(), StoreError> {
        ...
        let get_event = events
            ...
            .expect("Get event must have been recorded");
    ...
    async fn put_logs_audit_event() -> Result<(), StoreError> {
        ...
        let put_event = events
            ...
            .expect("Put event must have been recorded");
    ...
    async fn duplicate_put_logs_conflict() -> Result<(), StoreError> {
        ...
        let conflict_event = events
            ...
            .expect("second Put event must have been recorded");
