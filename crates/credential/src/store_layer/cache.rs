...
///
/// ```rust,ignore
/// use nebula_credential::store_layer::CacheConfig;
///
...
impl CacheStats {
    ...
    ///
    /// ```rust,ignore
    /// use nebula_credential::store_layer::CacheStats;
    ///
...
///
/// Sits below [`EncryptionLayer`](super::encryption::EncryptionLayer) in the
/// layer stack — cached values are **ciphertext**, never plaintext secrets.
///
/// # Examples
///
/// ```rust,ignore
/// use nebula_credential::store_layer::{CacheConfig, CacheLayer};
///
...
#[cfg(test)]
mod tests {
    use crate::{PutMode, StoreError, store::test_helpers::make_credential};

    use super::{super::InMemoryCredentialStore, *};

    #[tokio::test]
    async fn cache_hit_returns_cached() -> Result<(), StoreError> {
        let store = CacheLayer::new(InMemoryCredentialStore::new(), CacheConfig::default());
