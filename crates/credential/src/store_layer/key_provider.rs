//! `KeyProvider` — the seam between [`EncryptionLayer`](super::encryption::EncryptionLayer)
...
/// Source of the current encryption key for [`EncryptionLayer`](super::encryption::EncryptionLayer).
///
...
/// short, wrong-length, or dev-placeholder values.
///
/// # Examples
///
/// ```rust,ignore
...
impl FileKeyProvider {
    ...
    /// before any blocking or unbounded read would occur.
    ///
    /// # Errors
    ///
    ...
    pub fn from_path(path: impl AsRef<std::path::Path>) -> Result<Self, ProviderError> {
        ...
        // Reject anything that is not a regular file FIRST.
        if !metadata.is_file() {
            return Err(ProviderError::KeyMaterialRejected {
                reason: format!(
...
mod tests {
    ...
    /// mis-decrypting under the new key.
    #[test]
    ...
    /// directory) must refuse before any `read` call.
    #[test]
