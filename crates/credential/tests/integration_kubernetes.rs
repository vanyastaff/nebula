...
async fn test_k8s_list() {
    ...
    let all_ids = StorageProvider::list(&provider, None, &context)
        .await
    ...
    let filtered_ids = StorageProvider::list(&provider, Some(&filter), &context)
        .await
...
async fn test_k8s_update_credential() {
    ...
    StorageProvider::store(&provider, &id, data2.clone(), metadata2, &context)
        .await
