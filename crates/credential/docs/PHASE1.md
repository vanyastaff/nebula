...
| **5** | `Scheme` sealed family + per-protocol marker + `Slot<S>` (F3) — **design drafted, owner-fork pending** | Type design in DESIGN §17 ("Increment-5 type design"). Reseals/extends the foundational `nebula-core::AuthScheme` + the plugin open-world model, so it must reconcile with the existing `AuthPattern` (cosmetic, has `Custom`) + the sensitivity sub-traits. **Blocked on owner fork:** `SchemeFamily` vs `AuthPattern` (two-enums / fold / subsume). Slices once decided: **5a** `SchemeFamily` sealed enum + `AuthScheme::FAMILY` + migrate impls + macro; **5b** `Slot<S: AuthScheme>` bound + trybuild cross-protocol-reject + no-`Box<dyn>` arch-test; **5c** registration-time family-soundness check; **5d** plugin-marker open-world example. | core, credential, resource, macros | §17 F3 |
