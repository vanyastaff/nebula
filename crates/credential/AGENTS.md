...
- Use authority is the backend-authored admission epoch in `CredentialOperationStatus::Open`: a projection is admitted at the epoch read with its material (`CredentialGuardMetadata::admission_epoch`), and a consumer must not keep using it once the credential reports another. Display edits and retry-gate writes send `Preserve` (no bytes); the durable reauthentication decision sends `Advance { Unchanged }`; only resolved material sends `Advance { Replace }`.
