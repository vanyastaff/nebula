...
- Compiler epochs are closed: current 5/hash3 requires schema envelope v3 with schema policy v2. Historical 1/hash1, 3/hash2 and 4/hash2 or hash3 records remain readable evidence, not current checked plans. Their markerless Record/Any/Union envelopes stay v1 and Scalar envelopes stay v2. Never upgrade historical bytes, relabel a legacy empty Record as unit/null, or derive old epoch guards from the latest schema writer constant.
- Node root rules require a complete parameter object and no pending root checks from schema validation. References, deferred root rules, and passthrough root rules remain unsupported. Never discard an unresolved root obligation to admit a plan; presentation visibility does not create a contextual input requirement.
