...
- **Single capability source** — capability (refreshable / testable / revocable / interactive / dynamic) is read from the `nebula_credential::CredentialRegistry` `Capabilities` bitflag, computed once from sub-trait membership at `register::<C>()`. The former parallel `StateProjectionRegistry` (engine) + `CredentialDispatch` (runtime) flag tables and their sync invariant were removed in ADR-0088 D3 — one table cannot drift from itself. `DispatchOps<PS>` remains as the type-erased async **operation**-closure table (generic only over the pending-state store; the backend type param was a phantom marker and was dropped).
