# nebula-credential — HLD v1.8

> **Status:** Draft v1.8 — amended after two community conferences.
...
> **v1.7:** Community conference — capability supertraits (Aleksei/async-capability, Isabela/fintech, Tom/dylib, Yuna/compiler, Daniel/solo). `Refreshable`/`Interactive`/`Revocable`/`Testable` supertraits, registry closure table, `compile_error!` on missing impl, lifecycle docs §10.4.
> **v1.8:** Platform integration conference — Google/Preethi, GitHub/Marcus, AWS/Jordan, OCI/Yuichi, Stripe/Adaeze. `SigningScheme` supertrait, generate-on-resolve pattern, `CredentialEnvironment`, `DelegatedCredential`, `expires_at()` contract, B18.
...
### Community Conference — Capability Supertraits [v1.7]

5 developers: async framework vendor (Aleksei), fintech plugin author (Isabela), dylib plugin
system (Tom), Rust compiler contributor (Yuna), solo contractor/beginner (Daniel).

**Changes made:**

| Change | Type | Section |
|--------|------|---------|
| Capability supertraits: `Refreshable`, `Interactive`, `Revocable`, `Testable` | **[BREAKING]** | §3.1 |
| `RegistryEntry` closure table (`RefreshFn`, `ContinueResolveFn`, etc.) — registration-time erasure | **[ADDED]** | §3.1 |
| `CredentialCapabilities` struct confirmed runtime-queryable; survives FFI boundary | **[CONFIRMED]** | §3.1 |
| `Credential::REFRESHABLE` and siblings deprecated — derived from registry, removed in v2 | **[DEPRECATED]** | §3.1 |
| Split registration API: `register()` / `register_refreshable()` / `register_oauth2()` | **[ADDED]** | §3.1 |
| `MyCredential::register(&mut registry)` macro-generated convenience method | **[ADDED]** | §11 |
| `#[derive(Credential)]` emits `compile_error!` when capability declared but trait not impl'd | **[ADDED]** | §11 |
| §10.4 end-to-end lifecycle guide for refreshable credential (step 1–7) | **[ADDED]** | §10 |
| Note: closures captured at `register()` time; process restart required on State serde version bump | **[ADDED]** | §3.1 |
| OQ#35: `ErasedState` in refresh closure — behaviour on deserialization failure | **[DEFERRED]** | §14 |

### Platform Integration Conference [v1.8]

5 platform engineers: Google Cloud/Preethi (ADC, service accounts), GitHub/Marcus (App tokens,
OIDC), AWS/Jordan (STS, credential chain), OCI/Yuichi (request signing), Stripe/Adaeze
(webhook secrets, restricted keys).

**Changes made:**

| Change | Type | Section |
|--------|------|---------|
| `SigningScheme: AuthScheme` supertrait with blanket no-op impl — for OCI/AWS Signature v4 | **[ADDED]** | §3.1 |
| `sign_request(&mut Request<Bytes>)` injection path in action runtime (after scheme injection) | **[ADDED]** | §5 |
| "Generate-on-resolve" pattern: cache Scheme in State when HTTP call needed for projection | **[ADDED]** | §3.1 |
| `CredentialContext::environment()` → `CredentialEnvironment` trait (env vars, filesystem, HTTP) | **[ADDED]** | §3.2 |
| `MockEnvironment` in `test-support` — injectable env vars + mock HTTP for chain testing | **[ADDED]** | §10 |
| `DelegatedCredential` pattern: SDK-managed lifecycle, Nebula stores config only | **[ADDED]** | §3.4 |
| `AuthScheme::expires_at()` contract: `None` = no expiry (non-refreshable) vs unknown (refreshable) | **[ADDED]** | §3.1 |
| B18: `AwsAuth.session_token` audit — possibly missing or not `SecretString` | **[BUG]** | Bugs |
| OQ#36: `CredentialAccessor::verify_request()` for HMAC/webhook verification | **[DEFERRED]** | §14 |
| OQ#37: `OidcToken` scheme type with `audience` claim field | **[DEFERRED]** | §14 |
| OQ#38: `capabilities: Vec<String>` in `CredentialDescription` for scope attribution | **[DEFERRED]** | §14 |
| OQ#39: `sign_request` body type — `Bytes` vs `Vec<u8>` for mutable body access | **[DEFERRED]** | §14 |
...
pub trait Credential: Send + Sync + 'static {
    ...
    /// DEPRECATED [v1.7] — use capability supertraits instead.
    /// Survives as a derived value populated from RegistryEntry.capabilities.
    /// Will be removed in v2. Migration: read registry.entry(C::KEY)?.capabilities.refreshable.
    #[deprecated(since = "v1.7", note = "implement Refreshable supertrait instead")]
    const REFRESHABLE: bool = false;
    #[deprecated(since = "v1.7", note = "implement Interactive supertrait instead")]
    const INTERACTIVE: bool = false;
    #[deprecated(since = "v1.7", note = "implement Revocable supertrait instead")]
    const REVOCABLE: bool = false;
    #[deprecated(since = "v1.7", note = "implement Testable supertrait instead")]
...
#### Capability Supertraits [ADDED v1.7]

Capabilities are now declared by implementing a supertrait, not by setting a `const` flag.
The compiler enforces the contract: if you claim a capability, you must implement the method.
Supertraits are **registration-time proof tokens only** — they do not appear as bounds in
`CredentialResolver` or anywhere outside `CredentialRegistry::register_*()`. After registration,
dispatch goes through closures captured in `RegistryEntry`.

```rust
/// Implement for credentials that can renew their auth material without re-authentication.
/// Examples: OAuth2 (refresh_token), GitHub App tokens (re-sign JWT), GCP service accounts.
...
/// Implement for credentials that require user interaction to complete (OAuth2 PKCE,
/// device code flow, SAML). Stateless credentials use NoPendingState.
...
/// Implement for credentials that can be explicitly invalidated at the provider.
...
/// Implement for credentials that support a connectivity test without consuming them.
...
#### RegistryEntry closure table [ADDED v1.7]

At `register_*()` time, the capability closures are captured — capturing the full
deserialize → call → serialize chain for each capability. Call sites in `CredentialResolver`
remain capability-agnostic and dispatch through these closures.

```rust
// Type aliases for erased capability dispatch
type RefreshFn = Arc<dyn Fn(
    &[u8],                   // serialized state bytes (encrypted, passed through enc layer)
    &CredentialContext,
) -> BoxFuture<'static, Result<Vec<u8>, CredentialError>> + Send + Sync>;

type ContinueResolveFn = Arc<dyn Fn(
    PendingToken,
    InteractionInput,
    &CredentialContext,
) -> BoxFuture<'static, Result<ErasedResolveResult, CredentialError>> + Send + Sync>;

type RevokeFn  = Arc<dyn Fn(&[u8], &CredentialContext) -> BoxFuture<'static, Result<(), CredentialError>> + Send + Sync>;
type TestFn    = Arc<dyn Fn(&[u8], &CredentialContext) -> BoxFuture<'static, Result<TestResult, CredentialError>> + Send + Sync>;

pub struct RegistryEntry {
    pub key:          &'static str,
    pub capabilities: CredentialCapabilities,  // runtime-queryable — survives FFI
    // erased dispatch closures — None if capability not registered
    pub(crate) refresh_fn:          Option<RefreshFn>,
    pub(crate) continue_resolve_fn: Option<ContinueResolveFn>,
    pub(crate) revoke_fn:           Option<RevokeFn>,
    pub(crate) test_fn:             Option<TestFn>,
    // ...existing project_fn, description, etc.
}
```

> **Closure capture note:** closures are captured at `register()` time and include the serde
> logic for `C::State`. If `C::State` changes its serde schema (version bump), stale closures
> in a long-running process will fail to deserialize. **Process restart is required after
> credential type version bumps.**

#### Registration API [ADDED v1.7]

```rust
impl CredentialRegistry {
    /// Register a credential with no capabilities (static/non-refreshable).
    pub fn register<C: Credential>(&mut self) -> Result<(), RegistryError>;

    /// Register a refreshable credential. Compiler requires `impl Refreshable for C`.
    pub fn register_refreshable<C: Credential + Refreshable>(&mut self) -> Result<(), RegistryError>;

    /// Register an interactive credential.
    pub fn register_interactive<C: Credential + Interactive>(&mut self) -> Result<(), RegistryError>;

    /// Convenience for OAuth2: refreshable + interactive (most common combination).
    pub fn register_oauth2<C: Credential + Refreshable + Interactive>(&mut self) -> Result<(), RegistryError>;
}
```

`#[derive(Credential)]` generates `MyCredential::register(&mut registry)` which calls the
correct `register_*` variant based on declared attributes — plugin authors never call
`register_*` manually.
...
pub trait AuthScheme: Serialize + DeserializeOwned + Send + Sync + Clone + 'static {
    ...
    /// Returns the expiry time of this auth material, if known.
    ///
    /// Contract [ADDED v1.8]:
    /// - `None` from a **non-`Refreshable`** credential → **no expiry, never refresh**.
    ///   Framework will not schedule proactive refresh. Correct for webhook secrets,
    ///   static API keys, HMAC signing keys.
    /// - `None` from a **`Refreshable`** credential → expiry unknown; framework uses
    ///   `REFRESH_POLICY.early_refresh` window from last-known refresh time.
    /// - `Some(t)` → framework proactively refreshes `early_refresh` before `t`.
...
#### SigningScheme supertrait [ADDED v1.8]

For credentials where authentication requires signing each outgoing request (OCI, AWS
Signature v4, custom HMAC request signing). Bearer token credentials are unaffected —
blanket no-op impl means zero friction for the common case.

```rust
// In nebula-core — behind "request-signing" feature flag
pub trait SigningScheme: AuthScheme {
    /// Signs an outgoing HTTP request in-place.
    /// Called by the action runtime after credential injection, before dispatch.
    ///
    /// For OCI: computes RSA-SHA256 signature over canonical headers + body hash.
    /// For AWS v4: computes HMAC-SHA256 over canonical request.
    /// For bearer tokens: no-op via blanket impl below.
    fn sign_request(
        &self,
...
// Blanket no-op — all AuthScheme impls satisfy SigningScheme automatically.
// Override only for request-signing credentials.
impl<S: AuthScheme> SigningScheme for S {
    default fn sign_request(
        &self, _req: &mut http::Request<bytes::Bytes>,
    ) -> Result<(), SigningError> {
        Ok(())
    }
}
...
> **Note on `default fn`:** requires `specialization` feature (unstable). Stable alternative:
> provide `sign_request` as a non-default method with a no-op body and document that
> non-signing impls need not override it. Tracked in OQ#39.

#### "Generate-on-resolve" pattern [ADDED v1.8]

For credentials where obtaining `Scheme` requires a network call (GitHub App tokens, GCP
service accounts, any short-lived token generated from a signing key):

```
State  = { signing_material + cached_scheme + scheme_expires_at }
project(state)  → state.cached_scheme      ← sync, pure, no IO
resolve()       → generate first token via HTTP → write to State.cached_scheme
refresh()       → generate new token via HTTP  → update State.cached_scheme
```
...
pub struct GitHubAppState {
    ...
    private_key_pem: SecretString,            // long-lived signing material
    // cached scheme — populated by resolve(), updated by refresh()
    #[serde(with = "serde_secret")]
    cached_token: Option<SecretString>,
    token_expires_at: Option<DateTime<Utc>>,
}

impl Credential for GitHubAppCredential {
    fn project(state: &Self::State) -> InstallationToken {
        // Always sync — cached_token is guaranteed present after resolve()
        InstallationToken {
            token: state.cached_token.clone().expect("resolve() not called"),
            expires_at: state.token_expires_at,
        }
    }
}

impl Refreshable for GitHubAppCredential {
    async fn refresh(state: &mut Self::State, ctx: &CredentialContext)
        -> Result<RefreshOutcome, CredentialError>
    {
        // On key rotation: clear cache before regenerating
        state.cached_token = None;
        let new_token = generate_installation_token(state, ctx).await?;
        state.cached_token = Some(new_token.token);
        state.token_expires_at = Some(new_token.expires_at);
        Ok(RefreshOutcome::Updated)
    }
}
```

> **Rotation invariant:** always `state.cached_token = None` before generating new material
> during key rotation. Otherwise the old token survives until its own expiry window.
...
#### CredentialEnvironment [ADDED v1.8]

`CredentialContext` gains an `environment()` accessor for testable access to process
environment, filesystem, and HTTP. Required for implementing ADC chains (GCP), AWS credential
provider chains, OCI instance principals, and any credential whose `resolve()` needs to probe
...
pub trait CredentialEnvironment: Send + Sync {
    ...
    /// Returns the user home directory (for ~/.aws/credentials, ~/.config/gcloud, etc.)
    fn home_dir(&self) -> Option<PathBuf>;
    /// Shared HTTP client for metadata endpoints (IMDS, GCE metadata server, etc.)
    fn http_client(&self) -> &reqwest::Client;
}

pub struct SystemEnvironment;   // uses std::env + dirs + shared reqwest::Client
// Behind test-support feature:
pub struct MockEnvironment {    // injectable env vars + mock HTTP responses
...
pub struct CredentialContext {
    // ... existing fields ...
    environment: Arc<dyn CredentialEnvironment>,  // default: SystemEnvironment
}

impl CredentialContext {
    pub fn environment(&self) -> &dyn CredentialEnvironment { &*self.environment }
    // test-support: CredentialContext::with_mock_env(MockEnvironment::new())
}
```

Plugin authors call `ctx.environment().var("GOOGLE_APPLICATION_CREDENTIALS")` instead of
`std::env::var()` — identical logic, fully injectable in tests.
...
### 3.5 DelegatedCredential pattern [ADDED v1.8]

For credentials where the lifecycle is fully managed by a platform SDK (AWS SDK, GCP client
library, OCI SDK). Nebula stores configuration only — not credential material. The SDK owns
refresh, caching, and expiry.

**When to use:** AWS AssumeRole, GCP ADC, OCI Instance Principal, Azure Managed Identity —
any credential whose lifecycle is better managed by the platform's own credential vending
machinery. Avoids double-refresh conflicts between Nebula's `RefreshCoordinator` and the SDK.

**Trade-off:** Nebula does not emit `CredentialEvent::Refreshed` for SDK-managed credentials.
Resource pools receive no notification on token refresh. Use eager re-auth on each pool
checkout instead of event-driven re-auth.

```rust
/// Pattern template — not a base trait, a documentation pattern.
///
/// State  = configuration (role ARN, region, project ID) — NOT credential material
/// Scheme = obtained from SDK at project() time — SDK handles caching + refresh
/// REFRESHABLE = false — SDK owns the lifecycle
pub struct AwsStsAssumedRoleCredential;

pub struct AwsRoleConfig {
    pub role_arn: String,        // no SecretString — config, not secret
    pub session_name: String,
    pub region: String,
    pub external_id: Option<String>,
}

impl Credential for AwsStsAssumedRoleCredential {
    type State   = AwsRoleConfig;
    type Scheme  = AwsAuth;
    type Pending = NoPendingState;
    const KEY: &'static str = "aws.sts_assumed_role";

    async fn resolve(values: &ParameterValues, ctx: &CredentialContext)
        -> Result<ResolveResult<AwsRoleConfig, NoPendingState>, CredentialError>
    {
        let config = AwsRoleConfig::from_parameters(values)?;
        // validate connectivity — SDK will do the actual credential vending
        Ok(ResolveResult::Complete(config))
    }

    fn project(state: &Self::State) -> AwsAuth {
        // SDK call — returns cached credentials, refreshes internally if needed
        let creds = SDK_CREDENTIAL_CACHE.get_or_refresh(state);
        AwsAuth::from_sdk_credentials(creds)
    }
}
// Note: does NOT implement Refreshable — SDK manages refresh.
```
...
**Request signing injection [ADDED v1.8]:** For `SigningScheme` credentials, the action runtime
calls `sign_request()` after scheme injection and before HTTP dispatch:

```rust
// Action runtime — after credential injection
let scheme = ctx.credential_typed::<S>(id).await?;
// sign_request is a no-op for bearer tokens via blanket impl
scheme.sign_request(&mut outgoing_request)?;
client.execute(outgoing_request).await
```

OCI and AWS Signature v4 credentials override `SigningScheme::sign_request()`. All other
credentials get the blanket no-op — zero change to existing action code.
...
### 10.4 Complete lifecycle — refreshable credential [ADDED v1.7]

End-to-end narrative for plugin authors writing their first `Refreshable` credential:

1. **Implement `Credential`** — `KEY`, `description()`, `parameters()`, `project()`, `resolve()`
2. **Implement `Refreshable`** — `refresh()` called by `RefreshCoordinator` when token nears expiry
3. **Register** via derive: `#[credential(key = "..", refreshable)]` → generates `MyCredential::register(&mut registry)`  
   or manually: `registry.register_refreshable::<MyCredential>()`
4. **Engine detects approaching expiry** via `AuthScheme::expires_at()` + `REFRESH_POLICY.early_refresh` window
5. **`RefreshCoordinator` calls `refresh()`** — winner/waiter pattern, one refresh for N concurrent waiters
6. **`CredentialEvent::Refreshed` emitted** — resource pools re-auth on next checkout (lazy)
7. **Action continues with new token** — `project()` returns updated `Scheme`; the refresh is invisible

For the "generate-on-resolve" pattern (GitHub App, GCP service accounts): steps 2/4/5 apply
to `refresh()` calling the token generation endpoint, not a provider-side refresh grant.

### 10.5 Testing environment-dependent credentials [ADDED v1.8]

Credentials that use `ctx.environment()` (ADC chains, AWS credential chains) can be tested
without process environment mutation:

```rust
#[tokio::test]
async fn adc_prefers_env_var() {
    let env = MockEnvironment::new()
        .var("GOOGLE_APPLICATION_CREDENTIALS", "/path/to/sa.json")
        .with_file("/path/to/sa.json", SERVICE_ACCOUNT_JSON);

    let ctx = CredentialContext::with_mock_env(env);
    let result = AdcCredential::resolve(&ParameterValues::default(), &ctx).await;
    assert!(matches!(result, Ok(ResolveResult::Complete(_))));
}

#[tokio::test]
async fn aws_chain_falls_through_to_imds() {
    let env = MockEnvironment::new()
        // no AWS_ACCESS_KEY_ID — chain falls through to IMDS
        .mock_http("http://169.254.169.254/latest/meta-data/iam/...", IMDS_RESPONSE);

    let ctx = CredentialContext::with_mock_env(env);
    let result = AwsCredentialChain::resolve(&ParameterValues::default(), &ctx).await;
    assert!(matches!(result, Ok(ResolveResult::Complete(_))));
}
```

`MockEnvironment` is behind `test-support` feature. `SystemEnvironment` is the default in
production — uses `std::env`, `dirs`, and a shared `reqwest::Client`.
...
**Capability declaration and `compile_error!` [ADDED v1.7]:**

```rust
#[derive(Credential)]
#[credential(key = "slack_bot", name = "Slack Bot Token", refreshable, interactive)]
pub struct SlackBotCredential;

// Macro generates MyCredential::register() calling the correct register_* variant:
//   registry.register_oauth2::<SlackBotCredential>()
//
// If `refreshable` is declared but `impl Refreshable for SlackBotCredential` is missing,
// the macro emits:
//
// error: `SlackBotCredential` declares `refreshable` capability but does not implement
//        `nebula_credential::Refreshable`.
//        Add: impl Refreshable for SlackBotCredential { async fn refresh(..) { .. } }
```

The `compile_error!` is generated via a `const _: () = { /* trait bound assertion */ };`
block that produces a human-readable message pointing to the missing impl — not the internal
registry call site.
...
├── secret_string.rs        — SecretString (Zeroize + redacted Debug) + serde_secret
...
| 35 | **`ErasedState` in refresh closure — behaviour on deserialization failure** [v1.7] | Refresh closure captures `deserialize → C::refresh → serialize`. If `C::State` serde format has changed since `register()` (version bump without process restart), deserialization fails at runtime inside the closure. Current behaviour: propagates as `CredentialError::Crypto` or opaque serde error. Should be a distinct `CredentialError::StateVersionMismatch` variant with guidance to restart. |
| 36 | **`CredentialAccessor::verify_request()` for HMAC/webhook verification** [v1.8] | Stripe webhook and similar HMAC-verified inbound credentials need a standard verification path. Proposed: `accessor.verify_hmac(credential_id, body, signature) -> Result<(), VerifyError>` — internalises `HmacSecret.expose()` + HMAC-SHA256 + constant-time compare. Deferred: one platform requestor, insufficient signal for v1 API surface change. |
| 37 | **`OidcToken` scheme type with `audience` claim** [v1.8] | GitHub Actions OIDC tokens have `audience` claim required for validation. `BearerToken` has no audience field. Options: (a) new `OidcToken { token, audience, expires_at }` scheme type, (b) `BearerToken` gains optional `claims: Map<String, Value>`. Decision pending second OIDC use case. |
| 38 | **`capabilities: Vec<String>` in `CredentialDescription`** [v1.8] | Stripe restricted keys have scoped capabilities (e.g. `payment_intents:write`). Audit trail and UI would benefit from structured capability metadata on the credential type. Additive field on `CredentialDescription` — deferred until a second platform requestor. |
| 39 | **`sign_request()` body type: `Bytes` vs `Vec<u8>`** [v1.8] | OCI request signing requires SHA256 hash of request body for PUT/POST. `bytes::Bytes` is immutable — body must be fully buffered and accessible as `&[u8]`. Need to decide: `Request<Bytes>` (immutable, must clone for hash), `Request<Vec<u8>>` (mutable, easier hashing), or `Request<Body>` with buffering. Impacts `SigningScheme` trait signature — must resolve before `request-signing` feature is stabilised. |
...
| B18 | `AwsAuth` scheme type requires audit — `session_token: Option<SecretString>` possibly absent or typed as plain `String`; STS credentials are blocked without it | HIGH | `scheme/aws.rs` |