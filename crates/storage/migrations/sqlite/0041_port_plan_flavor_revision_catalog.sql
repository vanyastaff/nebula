...
CREATE TABLE port_worker_flavor_revisions (
    worker_flavor_id BLOB PRIMARY KEY
        CHECK (
            typeof(worker_flavor_id) = 'blob'
            AND length(worker_flavor_id) = 32
        ),
...
CREATE TABLE port_executable_plan_revisions (
    executable_plan_id BLOB PRIMARY KEY
        CHECK (
            typeof(executable_plan_id) = 'blob'
            AND length(executable_plan_id) = 32
        ),
    worker_flavor_id BLOB NOT NULL
        CHECK (
            typeof(worker_flavor_id) = 'blob'
            AND length(worker_flavor_id) = 32
        ),
...
CREATE TABLE port_execution_revision_refs (
    execution_id TEXT PRIMARY KEY
        CHECK (
            ...
            AND length(execution_id) = 30
            AND substr(execution_id, 1, 4) = 'exe_'
            AND substr(execution_id, 5, 1) GLOB '[0-7]'
            AND substr(execution_id, 5)
                NOT GLOB '*[^0123456789ABCDEFGHJKMNPQRSTVWXYZ]*'
        ),
    execution_contract_bundle_id BLOB NOT NULL
        CHECK (
            typeof(execution_contract_bundle_id) = 'blob'
            AND length(execution_contract_bundle_id) = 16
        ),
    executable_plan_id BLOB NOT NULL
        CHECK (
            typeof(executable_plan_id) = 'blob'
            AND length(executable_plan_id) = 32
        ),
    ...
    rollback_window_id BLOB
        CHECK (
            rollback_window_id IS NULL
            OR (
                typeof(rollback_window_id) = 'blob'
                AND length(rollback_window_id) = 16
            )
        ),
