...
#[tracing::instrument(name = "turn_recovery.list", skip_all,
    fields(backend = "postgres", limit, candidates = tracing::field::Empty))]
pub(super) async fn list(
    ...
    let rows = sqlx::query("SELECT a.execution_id, a.workspace_id, a.org_id, a.last_accepted_fencing_generation, e.lease_expires_at_ms, (EXTRACT(EPOCH FROM clock_timestamp()) * 1000)::bigint AS scan_now FROM port_execution_turn_acceptances a JOIN port_executions e ON e.id = a.execution_id AND e.workspace_id = a.workspace_id AND e.org_id = a.org_id JOIN port_execution_revision_refs r ON r.execution_id = a.execution_id WHERE r.reference_state = 'live' AND r.worker_flavor_id = $1 AND ($2::text IS NULL OR a.execution_id > $3) ORDER BY a.execution_id LIMIT $4")
...
#[tracing::instrument(name = "turn_recovery.accept", skip_all,
    fields(backend = "postgres", execution_id = handoff.execution_id(), outcome = tracing::field::Empty))]
pub(super) async fn accept(
    ...
    let result = async {
        ...
        let lease = sqlx::query("UPDATE port_executions SET lease_holder = $1, lease_expires_at_ms = $2, fencing_generation = $3 WHERE id = $4 AND workspace_id = $5 AND org_id = $6")
        ...
        let marker = sqlx::query("UPDATE port_execution_turn_acceptances SET last_accepted_fencing_generation = $1 WHERE execution_id = $2 AND workspace_id = $3 AND org_id = $4 AND last_accepted_fencing_generation = $5")
