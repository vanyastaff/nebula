...
-- token_hash is the 32-byte SHA-256 digest of the plaintext bearer token,
-- stored as raw BYTEA (not hex) so there is no collation or encoding
-- ambiguity on exact-match primary-key lookups.
-- UNIQUE(execution_id, node_key): ON CONFLICT DO NOTHING prevents duplicate
-- live tokens on crash re-drive.
