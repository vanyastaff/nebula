...
impl MembershipStore for PgMembershipStore {
    ...
    async fn upsert_org_member_guarded(
        ...
        // Read only after the organization lock is acquired: PostgreSQL's
        // READ COMMITTED snapshot must include a preceding lock holder's commit.
        let rows = sqlx::query(
    ...
    async fn remove_org_member_guarded(
        ...
        let workspace_ids = sqlx::query_scalar::<_, String>(
            "SELECT id FROM port_workspaces WHERE org_id = $1 ORDER BY id",
        )
        .bind(org_id)
        .fetch_all(&mut *tx)
        .await
