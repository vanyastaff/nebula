...
-- stored as raw BLOB (not hex) so SQLite BLOB exact-match cannot be confused
-- by text collations.  UNIQUE(execution_id, node_key) prevents duplicate live
-- tokens on crash re-drive (ON CONFLICT DO NOTHING at INSERT time).
