...
#[tracing::instrument(name = "control_turn.commit", skip_all, fields(backend = "sqlite", outcome = tracing::field::Empty))]
pub(super) async fn commit(
    ...
    let result = async {
        ...
        let Some(command) = sqlx::query("SELECT command, resume_target FROM port_control_queue WHERE id = ? AND execution_id = ? AND workspace_id = ? AND org_id = ? AND status = 'Processing' AND claim_generation = ?")
        ...
        let references: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM port_execution_revision_refs WHERE execution_id = ? AND worker_flavor_id = ? AND reference_state = 'live'")
