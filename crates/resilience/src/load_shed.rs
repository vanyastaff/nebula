...
/// Like [`load_shed`] but runs under a shared [`CallContext`].
///
...
/// *partially executed* operation is safe to abandon is the supplied
/// operation's own contract.
pub async fn load_shed_with_context<T, E, S, Fut, F>(
    context: &CallContext,
    should_shed: S,
    f: F,
) -> Result<T, CallError<E>>
where
    ...
    E: Send,
{
    load_shed_with_context_and_sink(context, should_shed, f, &NoopSink).await
}

/// Like [`load_shed_with_context`] but emits [`ResilienceEvent::LoadShed`]
...
/// *partially executed* operation is safe to abandon is the supplied
/// operation's own contract.
pub async fn load_shed_with_context_and_sink<T, E, S, Fut, F>(
    context: &CallContext,
    should_shed: S,
    f: F,
    sink: &dyn EventSink,
...
mod tests {
    ...
    #[tokio::test]
    async fn call_context_cancellation_skips_predicate_and_operation() {
    ...
    #[tokio::test]
    async fn call_context_deadline_bounds_operation() {
    ...
    #[tokio::test]
    async fn call_context_load_shed_emits_event() {
        let sink = RecordingSink::new();

        let result: Result<u32, CallError<()>> = load_shed_with_context_and_sink(
            &CallContext::empty(),
            || true,
            || async { Ok(42) },
            &sink,
