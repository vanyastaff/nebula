...
3. **Sensitive/Public/External только macro-enforced.** Взаимоисключаемость `SensitiveScheme`/`PublicScheme`/`ExternalScheme` обеспечивается только макросом; hand-rolled двойной impl компилируется. Дыра оставлена намеренно — трейты нигде не используются как bound, поэтому двойной impl инертен; альтернатива с sealed `type Sensitivity` рассмотрена и **отклонена** (sensitivity по-полевая, реальный zeroize-gate — `Zeroize`-bound на гварде). См. credential `DESIGN.md` §17, sensitivity-пункт.
