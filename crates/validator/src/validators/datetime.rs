...
impl DateTime {
    ...
    /// Creates a `DateTime` validator that only accepts `YYYY-MM-DD` format.
    ///
    /// Rejects inputs containing `T`, `Z`, or timezone offsets.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// use nebula_validator::foundation::Validate;
    ///
    /// let v = DateTime::date_only();
    /// assert!(v.validate("2026-02-16").is_ok());
    /// assert!(v.validate("2026-02-16T10:00:00").is_err());
    /// ```
    #[must_use]
...
impl Validate for DateTime {
    ...
    fn validate(&self, input: &str) -> Result<(), ValidationError> {
        ...
        // Date-only mode: strictly YYYY-MM-DD, reject anything with T or timezone
        if self.date_only_mode {
            if input.contains('T') {
                return Err(ValidationError::new(
                    "time_not_allowed",
                    "Date-only mode does not allow time components",
                ));
            }
            if input.contains('Z') || input.contains('+') {
                return Err(ValidationError::new(
                    "timezone_not_allowed",
                    "Date-only mode does not allow timezone markers",
                ));
            }
    ...
    fn metadata(&self) -> ValidatorMetadata {
        let description = if self.date_only_mode {
            "Validates date-only strings in YYYY-MM-DD format".to_string()
        } else {
            format!(
        ...
        ValidatorMetadata {
            ...
            tags: vec!["text".into(), "datetime".into(), "iso8601".into()],
...
mod tests {
    ...
    #[test]
    fn test_date_only_valid() {
        let v = DateTime::date_only();
        assert!(v.validate("2026-02-16").is_ok());
        assert!(v.validate("2000-01-01").is_ok());
        assert!(v.validate("1999-12-31").is_ok());
    }

    #[test]
    fn test_date_only_rejects_datetime() {
        let v = DateTime::date_only();
        assert!(v.validate("2026-02-16T10:00:00").is_err());
    }

    #[test]
    fn test_date_only_rejects_invalid_month() {
        let v = DateTime::date_only();
        assert!(v.validate("2026-13-01").is_err());
    }

    #[test]
    fn test_date_only_rejects_invalid_day() {
        let v = DateTime::date_only();
        assert!(v.validate("2026-02-30").is_err());
    }

    #[test]
    fn test_date_only_rejects_empty() {
        let v = DateTime::date_only();
        assert!(v.validate("").is_err());
    }

    #[test]
    fn test_date_only_rejects_no_zero_padding() {
        let v = DateTime::date_only();
        assert!(v.validate("2026-2-16").is_err());
    }

    #[test]
    fn test_date_only_rejects_wrong_order() {
        let v = DateTime::date_only();
        assert!(v.validate("16-02-2026").is_err());
    }

    #[test]
    fn test_date_only_rejects_timezone_z() {
        let v = DateTime::date_only();
        // Inputs with Z should be rejected even without T
        assert!(v.validate("2026-02-16Z").is_err());
    }

    #[test]
    fn test_date_only_leap_year() {
        let v = DateTime::date_only();
        assert!(v.validate("2024-02-29").is_ok()); // leap year
        assert!(v.validate("2023-02-29").is_err()); // not leap year
    }
}
