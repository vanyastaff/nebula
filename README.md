...
Every first-party deployment composition root in this workspace lives under `apps/`. `nebula-worker` is reusable runtime assembly for exact-flavor control consumption, accepted-turn recovery, and timer scanning; `apps/worker` chooses concrete storage, integrations, configuration, and process lifecycle. A downstream embedded host becomes a supported composition root only through the curated `nebula_sdk::embedded::RuntimeBuilder`; until that façade ships, embedding is not a supported deployment surface. The builder cannot replace or bypass runtime admission, aggregate write ownership, or tenant authority.
