...
- **Clean design > backward compatibility (для ядра).** Во внутренних слоях сломать чистый API лучше, чем нарастить на него адаптерный слой. "Shim" — ругательство. Но для публичного `nebula-sdk` стабильность контрактов и deprecation-периоды обязательны.
