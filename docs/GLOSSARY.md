...
| `SecretValue` | type | `implemented` | In-memory redacted secret (`String` or bytes) in `nebula-schema`; `Serialize` redacts. Optional `KdfParams` on `Field::Secret` can hash at resolve time. See [ADR-0034](docs/adr/0034-schema-secret-value-credential-seam.md). | §12.5, §3.5 |
