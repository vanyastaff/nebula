...
| `Plane B` | concept | `implemented` | Integration-internal credentials (per-action OAuth bearer tokens, per-API-call HTTP auth) — distinct from Plane A (workspace-bound user identity). Plane B credentials live in `nebula-credential` and travel through the engine's resolve/refresh paths. Defined in [ADR-0033](adr/0033-integration-credentials-plane-b.md). | §3.5, §13.2 |
| `Pending` (rotation FSM) | enum variant | `implemented` | Initial state of the rotation lifecycle FSM (`Pending → Validating → Committing → Committed → GraceExpired → Cleanup`). Distinct from `PendingDrain` in resource lifecycle. Lives in `nebula-credential::rotation::state`. | §13.2 |
| `Dynamic` (provider class) | trait | `implemented` | Capability sub-trait for credentials whose schema/scopes/audience can shift at runtime (e.g., per-tenant OAuth). One of the 5 capability sub-traits (`Interactive`, `Refreshable`, `Revocable`, `Testable`, `Dynamic`). Lives in `nebula-credential::contract::dynamic`. Per Tech Spec §15.4. | §3.5 |
| `sentinel` / `N=3-in-1h` | concept | `implemented` | Refresh-coordinator escalation threshold. When a holder marks the L2 claim row `sentinel = RefreshInFlight` then the process dies before clearing, the next refresh attempt observes the orphan sentinel; **3 sentinel events within 1 hour** escalate to `RefreshOutcome::ReauthRequired { reason: SentinelRepeated }` instead of further retry. Closes the n8n #13088 mid-refresh-crash class. Per [ADR-0041](adr/0041-durable-credential-refresh-claim-repo.md) and refresh-coordination sub-spec §6. | §13.2 |
| `herd-сценарий` (refresh stampede) | concept | `implemented` | Production race where multiple replicas, each holding the same credential nearing TTL, simultaneously attempt to refresh against the IdP. The two-tier refresh coordinator (L1 `L1RefreshCoalescer` in-process + L2 `RefreshClaimRepo` cross-replica via CAS+heartbeat) collapses concurrent attempts to one IdP call. Without this, rotated `refresh_token_v2` invalidates `refresh_token_v1` on a parallel replica → cascade auth failures. | §13.2 |
...
| `ResolvedValues` | type | `implemented` | Proof-token returned by `ValidValues::resolve` — indicates expression resolution + `Field::Secret` promotion (`FieldValue::SecretLiteral`) have succeeded. Read non-secret literals with `get`; use `get_secret` for `Field::Secret` (so plaintext never flows through the JSON `get` path). | §3.5, §4.5, §12.5 |
| `SecretValue` | type | `implemented` | In-memory redacted secret (`String` or bytes) in `nebula-schema`; `Serialize` redacts. Optional `KdfParams` on `Field::Secret` can hash at resolve time. See [ADR-0034](adr/0034-schema-secret-value-credential-seam.md). | §12.5, §3.5 |
