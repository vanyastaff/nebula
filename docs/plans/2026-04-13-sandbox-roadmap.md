...
### D5. Transport: plain UDS / Windows Named Pipes + line-delimited JSON
**No gRPC. No protobuf. No TLS. No self-signed certs.** We initially considered the go-plugin stack (tonic + prost + rustls + rcgen, ~65 transitive crates) but that pattern is designed for **cross-language plugin ecosystems** — Terraform plugins can be written in any language that speaks gRPC. nebula is deliberately **Rust-only** for safety reasons (memory safety, no runtime sandbox for a JS engine, auditability), so cross-language interop is explicitly not a goal and we don't pay for it.

Instead: plugin binary is started by the host, prints a short handshake line on its stdout (`NEBULA-PROTO-2|unix|/tmp/nebula-plugin-<uuid>.sock` on Linux/macOS or `NEBULA-PROTO-2|pipe|\\.\pipe\LOCAL\nebula-plugin-<uuid>` on Windows), binds the listener, waits for the host to dial exactly once, then runs the duplex event loop from slice 1a/1b over the accepted stream. Line-delimited JSON framing from slice 1a is preserved — zero change to the envelope shape.

**Authentication** is filesystem/pipe permissions:
- **Linux/macOS**: UDS is created in a directory with mode `0700` owned by the nebula user; the socket itself is `0600`. Any other user on the box gets `EACCES` at `connect()`. Same security boundary the SSH agent, systemd, and dbus rely on.
- **Windows**: pipe is created under the session-scoped `\\.\pipe\LOCAL\` namespace (other logon sessions cannot see it); a `SECURITY_ATTRIBUTES` DACL restricts to the creating user's SID. No TLS ceremony.

**Prior art** is LSP (Language Server Protocol — `rust-analyzer`, `clangd`, `gopls`, millions of users, 10+ years) and DAP (Debug Adapter Protocol — every serious debugger UI in existence). Both are plain bidirectional JSON-RPC over stdio or sockets. Zero gRPC, zero TLS, and proven to scale across platforms.

Plugin-SDK hides all transport details behind `run_duplex(handler)` — plugin authors see the same async trait they already see in slice 1a/1b.

**New workspace dependencies for this transport: zero.** Tokio's existing `net` feature gives us `tokio::net::UnixListener` on Unix and `tokio::net::windows::named_pipe::ServerOptions` on Windows; we add `net` to the tokio feature list in `plugin-sdk` and `sandbox` and use `#[cfg]` branches.

### D6. Lifecycle: long-lived per `(plugin_binary, credential_scope)` with Reattach
**Not per-invocation.** For workflow executions running hours to days, spawn-per-call is prohibitive. Plugin process is long-lived and scoped by `(plugin_binary_path, credential_scope_hash)` — same plugin binary with different credential bindings runs as different processes (prevents credential leakage). One plugin binary can offer multiple action keys; they all share the same process when the credential scope matches. **Reattach** means the engine can restart without killing in-flight workflows: supervisor persists `{pid, socket_path, binary_path}` to a state file, on engine start it tries to reconnect; if the socket is alive, reattach wins, otherwise respawn. This is the Nomad shape of go-plugin's lifecycle, without go-plugin's transport.
