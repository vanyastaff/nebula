...
### Task 4: Add Optional Registry Recording to the archived memory crate

The archived memory crate's stats structs (`AtomicCacheStats`, `MemoryStats`) serve dual purpose: internal cache logic (eviction decisions, pressure detection) AND observability. We keep the internal structs but add optional registry recording alongside.
...
Run: `cargo nextest run -p <archived-memory-crate> --features telemetry` (historical command)
Expected: ALL PASS
