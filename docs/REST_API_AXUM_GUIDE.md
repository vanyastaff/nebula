...
└──────────┘    └──────────────┘    └─────────────────┘
    │                    │                    │
    ▼                    ▼                    ▼
  storage / app:  Postgres, engine, runtime queue, worker loop
```

API зависит только от **контрактов** (traits из `nebula-storage` — WorkflowRepo, ExecutionRepo; очередь и сандбокс в `nebula-runtime`). Конкретные реализации (БД, очередь, движок) подставляются при сборке приложения (main/app crate). Так API остаётся тестируемым (mock-реализации) и не тянет за собой тяжёлые зависимости выполнения.
