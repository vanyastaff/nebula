# Archived From "docs/archive/nebula_parameters_updated.md"
...
**Purpose:** JSON editor with schema validation.
...
**Expression Examples:**
- Static: `{"event": "user_created", "data": {"id": 123}}`
- Expression: `"{{JSON.stringify({event: $json.eventType, userId: $json.user.id})}}"`

```rust
// Webhook payload with schema
let payload = JSONParameter::builder()
    .metadata(metadata)
    .schema(Some(json!({
        "type": "object",
        "properties": {
    ...
    .ui_options(JSONUiOptions {
        height: 12,
        auto_format: true,
        collapsible: true,
        show_schema_hints: true,
    })
    .build()?;
```
...
**Purpose:** Tag input with auto-completion and validation.
...
```rust
// Article tags
let article_tags = TagsParameter::builder()
    .metadata(ParameterMetadata::builder()
        .key("tags")
        .name("Article Tags")
        .description("Add relevant tags")
        .build()?)
    .ui_options(TagsUiOptions {
        suggestions: vec![
            "technology".into(),
            "business".into(),
            "programming".into(),
...
// SEO keywords
let keywords = TagsParameter::builder()
    .metadata(metadata)
    .ui_options(TagsUiOptions {
...
- `allow_custom` - allow creating new tags
...
**Purpose:** Cron format schedule configuration.
...
```rust
// Task schedule
let schedule = CronParameter::builder()
    .metadata(ParameterMetadata::builder()
        .key("schedule")
        .name("Schedule")
        .description("When to run this task")
        .build()?)
    .ui_options(CronUiOptions {
        show_builder: true,
        presets: vec![
            CronPreset {
                name: "Every hour".into(),
                expression: "0 * * * *".into(),
                description: "Runs at the start of every hour".into(),
            },
            CronPreset {
                name: "Daily at 9 AM".into(),
                expression: "0 9 * * *".into(),
                description: "Runs every day at 9:00 AM".into(),
            },
            CronPreset {
                name: "Weekly on Monday".into(),
                expression: "0 9 * * 1".into(),
                description: "Runs every Monday at 9:00 AM".into(),
            },
        ],
        show_next_runs: true,
...
**UI Options:**
- `show_builder` - show visual builder
- `presets` - preset options
- `show_next_runs` - show next executions
- `timezone` - timezone
...
**Purpose:** Webhook URL generation and management.
...
```rust
// GitHub webhook
let github_webhook = WebhookParameter::builder()
    .metadata(ParameterMetadata::builder()
        .key("webhook_url")
        .name("Webhook URL")
        .description("URL for GitHub to send webhooks")
        .build()?)
    .webhook_config(WebhookConfig {
        methods: vec![HttpMethod::POST],
        require_auth: true,
        path_suffix: Some("github".into()),
        auto_regenerate: false,
    })
    .build()?;

// Generic webhook
let generic_webhook = WebhookParameter::builder()
    .metadata(metadata)
    .webhook_config(WebhookConfig {
...
- `auto_regenerate` - automatic URL regeneration
...
### Removed: JSONParameter

**Why removed:** Instead of free-form JSON editing, use structured approaches:

**For structured data:** Use `ObjectParameter` + `ParameterList` combinations
```rust
// Instead of JSONParameter, build structure with typed parameters
...
### Removed: TagsParameter and CronParameter  

**Tags Alternative:** Use `MultiSelectParameter` with `creatable: true` or `TextParameter` with comma separation
**Cron Alternative:** Use `TextParameter` with cron validation
...
let tag_list = ParameterList::builder()
    .metadata(ListMetadata::new("tags", "Tags"))
    .item_template(
        Parameter::Text(
            TextParameter::builder()
