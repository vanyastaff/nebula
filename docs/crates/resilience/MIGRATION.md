...
- **Compose layer trait signatures changed (breaking)**
  - **Old behavior:** `ResilienceLayer::apply` and `LayerStack::execute` accepted `BoxedOperation<T>` by value.
  - **New behavior:** operations are passed by reference (`&BoxedOperation<T>`) and layer APIs accept optional cooperative cancellation context.
  - **Migration steps:**
    - Update custom layer implementations to the new signature:
      - `apply(&self, operation: &BoxedOperation<T>, next: &(dyn LayerStack<T> + Send + Sync), cancellation: Option<&CancellationContext>)`
    - Update direct stack execution call sites to pass references:
      - `stack.execute(&boxed_operation).await`
    - If cancellation integration is needed, call `execute_with_cancellation(&boxed_operation, Some(&ctx))`.

- **ResilienceManager supports cooperative cancellation**
  - **Old behavior:** manager retry backoff could continue sleeping after shutdown/cancel requests.
  - **New behavior:** `ResilienceManager::execute_with_cancellation(...)` interrupts in-flight retry backoff and returns `ResilienceError::Cancelled`.
  - **Migration steps:** pass a shared `CancellationContext` from runtime/engine orchestration paths where fast shutdown is required.

- **RetryStrategy adds cancellation-aware resilient API**
  - **Old behavior:** only `execute_resilient(...)` existed; backoff sleep was not externally cancellable.
  - **New behavior:** `execute_resilient_with_cancellation(...)` supports cooperative cancellation before attempts, during attempts, and during backoff waits.
  - **Migration steps:** for long-running workflows and shutdown-sensitive paths, prefer `execute_resilient_with_cancellation(...)` and propagate a `CancellationContext`.
