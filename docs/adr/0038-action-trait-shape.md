...
**Accepted 2026-04-25** — drafted 2026-04-24 alongside ADR-0039 / ADR-0040 as the 3-ADR set for the nebula-action redesign cascade ([Strategy §6.2](../ARCHIVE.md)). Status moved from `proposed` → `accepted` 2026-04-25 after Phase 6 Tech Spec FROZEN CP4 ratification (tech-lead 11c freeze ratification).

## Context

The current `nebula-action` integration surface uses `#[derive(Action)]` (`crates/action/macros/src/derive.rs`) which cannot perform field-type rewriting — derives observe fields by name and type but emit code adjacent to the struct, not over it. The credential CP6 spec ([credential Tech Spec §2.7 line 486-528](../ARCHIVE.md), [§3.4 line 807-939](../ARCHIVE.md)) specifies a typed credential surface — `CredentialRef<C>` field-level handles, dyn-position translation `CredentialRef<dyn ServiceCapability>` → `CredentialRef<dyn ServiceCapabilityPhantom>` per ADR-0035 — that requires field-zone rewriting, not just adjacent emission.

Phase 4 spike (commit `c8aef6a0`, [spike NOTES](../ARCHIVE.md) §1.4 + §3 finding #3) confirmed two structurally-distinct enforcement mechanisms for the "bare `CredentialRef<C>` outside `credentials(...)` zone" contract:
...
1. Aligns nebula-action with the credential CP6 typed surface ([credential Tech Spec §2.7 / §3.4](../ARCHIVE.md)) — `CredentialRef<C>` field handles, `SlotBinding` registration, `SchemeGuard` RAII flow through one cohesive vocabulary.
...
1. **Hard break on `#[derive(Action)]` for plugin authors.** The 7 reverse-deps (Strategy §6.1) must migrate; codemod ships in cascade per [scope decision §1.6](../ARCHIVE.md). Acceptable per `feedback_hard_breaking_changes`.
...
- [Strategy Document](../ARCHIVE.md) — §3.2 placement lock; §4.3.1 macro modernization; §6.2 ADR roadmap; §6.8 B'+ contingency activation criteria (fallback path for this decision).
- [Scope decision](../ARCHIVE.md) — §1 chosen scope (Option A'); §1.6 plugin ecosystem migration design.
- [Spike NOTES](../ARCHIVE.md) — §1.4 Probe 3 result (option (b) type-system-enforceable); §3 finding #3 (dual enforcement layer rationale); commit `c8aef6a0`.
...
- [Credential Tech Spec](../ARCHIVE.md) — §2.7 line 486-528 macro translation; §3.4 line 807-939 dispatch narrative.
