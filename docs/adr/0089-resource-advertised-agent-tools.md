...
- **`AnyManagedResource`** (`registry.rs:77`): the **sealed type-erased**
  registry interface — the natural home for an erased `tools_erased()`
  discovery hook.
...
### D1 — `ResourceTools` internal trait (sealed, `&self`, RPITIT)

The trait sits flush with the existing rotation-hook pattern — another `&self`
capability read off the immutable descriptor, RPITIT exactly as `Resource`
already does. It is **sealed** so methods can be added later without a semver
major.
...
pub trait ResourceTools: sealed::Sealed + Send + Sync {
    ...
    /// them (discovery-time gating — §Security).
    fn tools(&self, scope: &TenantScope) -> Vec<ToolDefinition>;

    /// Invoke one operation by name. RPITIT — no `Box<dyn Future>` on the happy
    /// path, mirroring `Resource::create`. `&self` borrow ties the call to the
    /// guard's lifetime (the tool cannot outlive the live resource).
    fn invoke(
        &self,
        name: &str,
...
**Static public trait + object-safe erased twin.** RPITIT `async fn` is not
`dyn`-compatible, but an agent loop needs a heterogeneous tool set across
differently-typed resources. Resolve the same way the rest of the crate already
erases: keep `ResourceTools` (Candidate A — static, zero-heap `invoke`, matches
typed `ResourceGuard<R>` field access) as the **public** trait, and wire
discovery through a `pub(crate) trait ErasedResourceTools` whose `invoke`
returns `Pin<Box<dyn Future + Send>>`, blanket-`impl<T: ResourceTools>`, hung off
`AnyManagedResource::tools_erased()` (matches the existing sealed registry
erasure at `registry.rs:77`; one heap alloc per erased invoke). This is the
rmcp + erased-serde idiom [R2].
...
|---|---|---|
| Business | **`nebula-resource`** | `ResourceTools` + `ErasedResourceTools` + `ToolDefinition`/`ToolAnnotations` + `ToolError` + `ToolInvokeCx` + `tools_erased()` on `AnyManagedResource`. Lifecycle-adjacent capability only. |
...
tool/annotation struct, RPITIT (safe on the 1.95 floor).

| Version | What lands | Why now | Depends on | Risk | ADR action |
|---|---|---|---|---|---|
| **v0.x — experimental** | `ResourceTools` (sealed, `&self`, RPITIT) + erased twin; `ToolDefinition`/`ToolAnnotations` (`#[non_exhaustive]`); typed `ToolError`; `tools_erased()` on `AnyManagedResource`; **discovery-only** proven on one read-only resource (Postgres `pg.query`). `nebula-agent` crate created; minimal loop fetches a catalog off a guard. | Stabilise the contract before the macro and before any consumer; read-only first removes the destructive-gating surface from v0. | M12.4 bind-population; `nebula-schema` | Trait churn — bounded by sealing + `#[non_exhaustive]`. AFIT-in-`dyn` gap — solved by the erased twin. | This ADR (amends 0057). |
| **v0.x+1 — security hardening** | `ToolInvokeCx` attenuation; credential-generation TOCTOU check; tenant-scope discovery filter; `ToolInvoked` audit + observability triple; destructive annotation + invoke-time allowlist gate; secondary `AgentTool` provider folded into one `ToolRegistry`. | Security is DoD — a writable tool surface cannot reach an LLM without attenuation + audit + gating. | v0.x; `nebula-tenancy`; `nebula-eventbus`. | Confused-deputy / TOCTOU if attenuation is incomplete. | Amend (gating + audit invariants). |
| **1.0 — hardened freeze** | Freeze `ResourceTools` + `ToolDefinition` in the SDK / plugin-sdk contract; `#[resource_tools]` derive macro; exercise `output_schema`; add `fn resource_capabilities() -> ResourceCapabilities` (default `NONE`, additive on the sealed trait). | Freeze the surface in the 1.0 contract; the macro lowers author cost once the contract is proven. | M12 + M13; `MATURITY.md` stable surface (M14.1). | Macro freezing an unproven contract — mitigated by shipping hand-written v0 first. | Macro design note; capabilities flag additive. |
| **1.1+ — deferred (NOT in 1.0)** | Dynamic tool sets (`listChanged` via eventbus); streaming tool output; wire-MCP **export** to external clients (separate `nebula-mcp-bridge` crate, `tools()` → MCP JSON); elicitation. | MCP client `listChanged` support is sparse (2026 target = static lists); streaming waits on AFIT-in-`dyn` ecosystem stabilisation; wire export must stay out of the internal crate. | 1.0 contract stable; a consumer for each. | Premature dynamism / boundary erosion. | Wire-bridge = its own ADR + its own crate. |
...
- **Static `tools()` vs async discovery:** sync `fn tools(&self, scope) ->
  Vec<…>` (cheap, re-callable) vs `async fn discover()` (only if enumeration does
  I/O). Recommendation: sync; defer async discovery to the 1.1 `listChanged`
  line.
- **`ToolError` — new type vs reuse `ActionError`:** a dedicated `ToolError`
  ...
  `nebula-resource` to the action error taxonomy. Recommendation: new `ToolError`
  in `nebula-resource` with `From<ToolError> for ActionError` at the agent seam.
  **Open.**
...
- **Agent declaration surface (DX):** a new `#[derive(AgentAction)]` vs an agent
  *variant* of the existing `#[derive(Action)]` that auto-folds every
  ...
  Context). Recommendation: extend
  `#[derive(Action)]` so the resource/credential/tool resolution share one
  emitter. **Open.**
...
- New surface area is bounded and sealed; `#[non_exhaustive]` + RPITIT + sealing
  keep the v0→1.0→1.1 evolution semver-additive.
...
- [R2] rmcp (official Rust MCP SDK) tool-router idiom — https://docs.rs/rmcp/0.8.5/rmcp/ ; sealed traits — https://predr.ag/blog/definitive-guide-to-sealed-traits-in-rust/
