...
### §2.7 `#[action_phantom]` macro translation

Action structs have fields typed `CredentialRef<dyn BitbucketBearer>` in user-facing syntax. `#[action_phantom]` attribute macro **rewrites silently** to `CredentialRef<dyn BitbucketBearerPhantom>` in generated code.

**Naming.** The rewriter is `#[action_phantom]`, **not** `#[action]`. `#[derive(Action)]` declares `#[action(key = ..., name = ..., ...)]` as an inert helper attribute; reusing the bare `#[action]` name for the rewriter put two different things called `#[action]` on the same struct, which collided as a footgun if the macro was `use`d into scope. The renamed `#[action_phantom]` keeps the helper-attribute namespace clean and signals what the macro does (phantom-shim rewrite for action structs). Amendment 2026-04-26 post Stage 4 review.
...
#[action_phantom]
#[derive(Action)]
#[action(key = "bb.repo.fetch", name = "Fetch repo", description = "...")]
...
// + ActionMetadata impl per §3.4 dispatch (generated by the derive).
```
...
#[action_phantom]
#[derive(Action)]
#[action(key = "slack.post", name = "Post message", description = "...")]
pub struct SlackPostMessageAction {
    pub slack: CredentialRef<SlackOAuth2Credential>,
}
```

**Output:** identical (no phantom translation — `SlackOAuth2Credential` is `Sized`, not `dyn`). Pattern 1 doesn't strictly require `#[action_phantom]` since there are no `dyn` fields; users may omit it.
...
The `#[action_phantom]` macro-rewritten field `CredentialRef<dyn BitbucketBearerPhantom>` forces the supplied credential type `T` to satisfy `BitbucketBearerPhantom`, which requires `T: BitbucketBearer` via the sealed_caps blanket chain.
...
Engine does NOT pass `&dyn BitbucketBearerPhantom` to the action body. Instead, the `#[action_phantom]` + `#[derive(Action)]` macros generate slot metadata:
...
- `tokio::test` runtime.
- `trybuild` — compile-fail probes for phantom-shim and `#[action_phantom]` macro diagnostics.
...
1. **Compile-time slot metadata.** `#[action_phantom]` + `#[derive(Action)]` macros emit `SlotBinding` per credential field (§3.4 step 2), with `slot_type` as one of three `SlotType` variants — `Concrete { type_id }` (Pattern 1), `ServiceCapability { capability, service }` (Pattern 2), or `CapabilityOnly { capability }` (Pattern 3).
...
3. **Macro cross-check** (§3.5). `#[action_phantom]` macro verifies that Resource's `AcceptedAuth` bound matches Action's `CredentialRef<...>` bound via trait-resolution where-clause.
