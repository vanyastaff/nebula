...
2. **ADR amendment.** Any of ADR-0035 / ADR-0038 / ADR-0039 / ADR-0040 moves from `accepted` to `superseded` or undergoes a non-trivial amendment.
...
**G1 — Credential CP6 vocabulary adoption (closes CR1, CR5–CR10).** Action surface adopts the credential Tech Spec CP6 typed-credential paradigm: `CredentialRef<C>` field-level handles, `SlotBinding` registry registration with HRTB `resolve_fn`, `SchemeGuard<'a, C>` RAII at the action body. `ctx.credential::<S>(key)` / `credential_opt::<S>(key)` materialize through macro-emitted slot bindings. Time-to-first-successful credential-bearing action drops from 32 minutes (current; see [`02-pain-enumeration.md`](../drafts/2026-04-24-nebula-action-redesign/02-pain-enumeration.md) §1, dx-tester reference) to <5 minutes (Strategy §1 target). Per Strategy §3.1 component 2 + ADR-0038 §Decision.

**G2 — Macro emission correctness with regression harness (closes CR2, CR8, CR9, CR11).** `#[derive(Action)]` is replaced by `#[action]` attribute macro per ADR-0038; macro emits regression-covered tokens for the four action shapes; `crates/action/macros/Cargo.toml` gains `[dev-dependencies]` block with `trybuild` + `macrotest`; six probes from Phase 4 spike commit `c8aef6a0` port forward as the production harness baseline (per ADR-0039 §4 table). Three independent agents hitting the same `parameters = Type` emission bug becomes structurally impossible because regression coverage exists.
...
**G4 — Sealed DX tier ratification (per ADR-0040).** Five DX traits — `ControlAction`, `PaginatedAction`, `BatchAction`, `WebhookAction`, `PollAction` — become sealed per ADR-0040 §1 (sealed-trait pattern via per-capability inner sealed trait, following ADR-0035 §3 convention). Canon §3.5 line 82 revises per ADR-0040 §2 to enumerate the DX tier explicitly; the 4-primary trait family is preserved. Sealing closes the §1(c) governance drift.
...
**N1 — Resource integration deeper than ADR-0035 §4.3 rewrite obligation.** Resource-side scope (resource crate redesign, `Resource::on_credential_refresh` full integration, resource cluster-mode coordination) is OUT. Action's responsibility ends at the `ResourceAction::Resource` associated type binding + `CredentialRef<C>` field-zone rewrite per ADR-0038 §3 + the consumer-side completion of ADR-0035's two-trait phantom-shim contract (ADR-0038 §Decision item 4). Strategy §3.4 row "`Resource::on_credential_refresh` full integration" (line 173) names the home: absorbed into resource cascade or co-landed with credential CP6 implementation.
...
The supertrait bound on `ActionSlots` makes "no `ActionSlots` impl → no `Action` impl" structural — Probe 3's type-system enforcement layer per ADR-0039 §2 (line 67-70). `Send + Sync + 'static` is required for handler erasure; spike Iter-1 §1.7 confirmed the bound chain compiles via `assert_is_action::<A>()` across all three iter-2 actions.

**`ActionMetadata`** (the return type of `Action::metadata`) is defined at `crates/action/src/metadata.rs` (current shape; field-set lock is CP3 §7 scope per Strategy §5.1.1). The macro emits the metadata literal from `#[action(...)]` attribute fields per ADR-0039 §1.
...
/// `&'static [SlotBinding]` literal — see §3.1 + ADR-0039 §1.
...
The four primary traits carry the dispatch-shape contract per PRODUCT_CANON §3.5 line 82 (revised by ADR-0040 §2). Each trait uses **RPITIT** (`-> impl Future<Output = ...> + Send + 'a`) to express the body's async return — verified via spike `final_shape_v2.rs:213-262`.
...
**ADR composition.** This amendment introduces no new ADR. ADR-0024 already governs the policy; ADR-0038 (trait shape) and ADR-0039 (macro emission) are unaffected — neither ADR locks the `*Handler` per-method async return shape; both lock the *trait family* and *macro emission for the action struct*, which are orthogonal to the `*Handler` annotation choice. ADR-0035 phantom-shim composition is preserved (the phantom-shim contract is field-shape-level, not method-signature-level).
...
Engine dispatches on the 4-variant enum per current `crates/action/src/handler.rs:39-50` (preserved post-modernization — the variant set is the canon §3.5 dispatch core, and ADR-0040 §1 confirms the DX tier "erases to primary" at runtime).

**No `Control` variant.** `ControlAction` is sealed (§2.6) and erases to `Stateless` via adapter at registration time; the engine never sees a `Control` variant. This is the load-bearing property ADR-0040 §1 / §2 ratifies — adding a primary dispatch trait requires canon revision (§0.2 trigger), but adding a sealed DX trait does NOT (per ADR-0040 §2 revised wording).

### §2.6 Five sealed DX traits per ADR-0040
...
The DX tier wraps the primary dispatch traits with authoring-friendly shapes that erase to `Stateless` / `Stateful` / `TriggerHandler` at dispatch. Each DX trait is **sealed** per ADR-0040 §1 — community plugin crates may NOT implement it directly; they go through the underlying primary trait + adapter, OR (for Webhook/Poll) through the `#[action]` macro attribute zones that emit the sealed adapter automatically.
...
**Community plugin authoring path** (per ADR-0040 §1 + ADR-0040 §Negative item 4). External plugin crates do **NOT** implement any sealed DX trait directly. The five DX shapes — pagination, batch, control-flow, webhook, poll — are authored via:
...
**`SchemeGuard<'a, C>` is `!Clone`** per credential Tech Spec §15.7 + ADR-0039 §3 — the qualified-syntax probe `<SchemeGuard<'_, C> as Clone>::clone(&guard)` is mandated at test time to catch the auto-deref Clone shadow (where unqualified `guard.clone()` resolves to `Scheme::clone` via `Deref`, defeating the `!Clone` invariant silently). The non-Clone receipt is load-bearing for the cancellation-zeroize invariant in §3.4 — a clonable guard would let the action body retain a copy past scope, defeating the `tokio::select!` cancellation discipline. Probe discipline is enforced at CP2 §8 testing scope.
...
#### §2.9.1c User pushback during post-freeze re-examination (Q3, verbatim) and resolution

**Amended-in-place 2026-04-25** post-freeze (rationale-tightening only — no §2.2 signature change; no status qualifier change per §15.9.6). The user re-raised §2.9 with concrete consumer evidence:

> «n8n has 70+ trigger nodes. Each one exposes (1) Input schema for UI form generation, (2) Output schema for downstream node type-checking, (3) filter/parameter validation. Examples: Telegram Trigger configures `allowed_updates` + `restrict_to_chat_ids`, emits `TelegramUpdate`; GitHub Trigger configures `repository` + `events`, emits `GitHubPayload`; Schedule Trigger configures `interval` + `cron`, emits `ScheduledTick`. Under workflow-engine paradigm, trigger has typed Configuration AS Input AND typed EventStream AS Output at the trait level. §2.9.6 point 2 ("no current consumer for typed I/O reflection") is incorrect — UI generation per n8n parity is a concrete consumer.»

**The user's evidence is concrete and correct on its axis. The §2.9 verdict still stands on its axis. The refinement names the missed axis: schema-as-data vs schema-as-trait-type as carriers for schema reflection.**

**Two reflection-carrier axes exist; n8n consumers live on the data axis.**

1. **Schema-as-data axis (runtime, JSON-reflected).** What `ActionMetadata.base.schema: ValidSchema` / `ActionMetadata.inputs: Vec<InputPort>` / `ActionMetadata.outputs: Vec<OutputPort>` provides (`crates/action/src/metadata.rs:98-118` + `crates/action/src/port.rs:121-128, 185-195`). Consumers: UI form generation, engine port routing, downstream node type-checking via JSON-schema compatibility, validator filter rules, manifest export. **Universal across all 4 trait variants today** — the `parameters = T` macro zone produces `ActionMetadata.base.schema = <T as HasSchema>::schema()` regardless of trait variant; `ActionMetadata.inputs` / `outputs` declare port topology regardless of trait variant.
2. **Schema-as-trait-type axis (compile-time, monomorphized, type-system-mediated).** What `type Input` / `type Output` on a trait provides. Consumers: Rust generic code that pattern-matches on action types at compile time, type-system-driven dependency analysis, monomorphic adapter construction (`fn collect<T: Action<I, O>>` aggregation by type identity). Under this axis, consolidation would mean "every trait variant declares `type Input` / `type Output` uniformly."

**n8n's three named consumers all live on the schema-as-data axis.** n8n's `INodeTypeDescription` interface (the n8n equivalent of `ActionMetadata`) carries `properties` (input form), `inputs` / `outputs` arrays (port topology), and per-property `displayOptions` (filter validation). n8n is JS/TS at runtime — it does NOT use TypeScript generic associated types for these; schema reflection lives in `INodeTypeDescription` runtime arrays as data. The Telegram / GitHub / Schedule examples cited all expose schemas through `INodeTypeDescription.properties` and `INodeTypeDescription.outputs`, not through compile-time generic parameters.

**Even if Nebula committed to n8n surface parity, the consumers would not require trait-level Input/Output.** Nebula already provides the schema-as-data surface universally, including for TriggerAction:

- **Configuration / Input form generation.** `parameters = T` macro zone produces `ActionMetadata.base.schema = <T as HasSchema>::schema()`. Available for all 4 trait variants. Discoverability convenience layer (`for_stateless` / `for_stateful` / `for_paginated` / `for_batch` per `crates/action/src/metadata.rs:166-222`) currently has no `for_trigger::<A>()` helper — that is open item §2.9-1, closed at §15.6 as a metadata-builder convenience question. The underlying `with_schema(<RSSConfig as HasSchema>::schema())` builder is the universal mechanism that works without `type Input` on TriggerAction.
- **Output / downstream type-checking.** `ActionMetadata.outputs: Vec<OutputPort>` carries port-typing data per `crates/action/src/port.rs:185-195`; engine routing reads this. JSON-schema compatibility on port matching is the runtime check. TriggerAction declares its output port (the event-stream channel) at metadata-construction time, identically to how Stateless declares its data-output port. No trait-level `type Output` is required for this consumer.
- **Filter / parameter validation.** `ValidSchema` constraints in the parameters schema cover this universally; the validator reads `ActionMetadata.base.schema` regardless of trait variant.

**Nebula's stated competitive position disclaims n8n parity at canon level.** Per [`docs/COMPETITIVE.md`](../../COMPETITIVE.md):

> Line 29: «Competitive dimension (do not dilute): Reliability and clarity of execution as a system, plus DX for integration authors — not feature parity with n8n/Make on day one, and **not** a surface-area race in v1.»
>
...
n8n surface parity is not a goal Nebula commits to. The user's framing positions n8n parity as required; canon positions it as explicitly not pursued.

**Why a typed `type Input = TelegramConfig` / `type Output = TelegramUpdate` on TriggerAction would NOT enable any consumer beyond what schema-as-data already provides.** The engine's runtime dispatch goes through `Arc<dyn TriggerHandler>` over `serde_json::Value`-typed events (§2.4, §2.5). Cross-action type-checking at the workflow-graph layer happens through `OutputPort` / `InputPort` port-typing + JSON-schema compatibility, NOT through Rust's compile-time type system — the engine erases through JSON by design. Adding trait-level associated types does not pierce the JSON erasure; it adds a parallel reflection surface on the trait-type axis without a consumer that requires compile-time over runtime reflection. TriggerAction's existing `<Self::Source as TriggerSource>::Event` projection (per spike `final_shape_v2.rs:260` + §2.2.3 line 209) already provides compile-time event-type access for any future `T: TriggerAction` consumer that requires it — not blocked by the absence of `type Output` on TriggerAction.

**Refactor proposal `handle(.., input, event)` evaluation.** The user asked whether `handle` should accept `Self::Input` as a method parameter (configuration as a per-call parameter). REJECT for four converging reasons:

1. **Configuration lifecycle is per-instance, not per-event.** Configuration is set once at registration, read on every event from `&self`. Threading per-event would force engine reconstruction per dispatch — wasteful and lifecycle-incorrect (Strategy §3.1 component 7 ratifies the registration-once-read-many trigger lifecycle).
2. **Symmetry would force the same shape on three other traits where it does not fit.** StatelessAction/StatefulAction/ResourceAction's `type Input` already represents per-dispatch user-supplied data, not configuration. Threading configuration separately doubles the input-axis on three of four traits.
3. **ADR-0035 phantom-shim composition operates on `&self` field-zone rewriting** (per ADR-0035 §4.3). Configuration in `&self` fields IS the rewrite target for credentials/resources zones. Moving configuration to a method parameter makes the phantom-shim contract structurally meaningless for trigger-configuration credentials (e.g., `CredentialRef<dyn KafkaSaslPhantom>` for a Kafka trigger's auth would have nowhere to live as a method-parameter type).
4. **n8n parallel does not thread configuration as a method parameter.** n8n's `INodeType.execute` reads configuration via `this.getNodeParameter()` from the node instance — parallel to Nebula's `&self`-field read. The proposed refactor does not match the n8n parallel the user invokes.

**Refined verdict (Q3 post-freeze).** REJECT (refined three times) preserved. The schema-as-data vs schema-as-trait-type axis distinction is the missed framing the prior two REJECTs did not name explicitly enough; it is added now. n8n consumer evidence is acknowledged as concrete and correct — it lives on the data axis, where Nebula's surface is universal and goal-aligned with `docs/COMPETITIVE.md` line 41 typed-Rust-integration-contracts bet (not n8n surface parity). Trait-level Input/Output consolidation does not unblock data-axis consumers; it adds a parallel surface on the trait-type axis without a current consumer. The four trait shapes from `final_shape_v2.rs:209-262` remain the signature-locking source.

**Open items re-confirmation.** §2.9-1 (CP3 §7 `for_trigger::<A>()` helper) carry-forward closed at §15.6 — universal `with_schema` builder pattern preserved; no helper added. The user's Q3 framing reinforces the §15.6 closure rationale: schema-as-data is universal, the helper is a discoverability convenience question for a future CP3 metadata-field-set lock, not a trait-shape question.
...
**REJECT consolidation. Status quo (Option C) preserved.** (Rationale tightened across three iterations: CP2 2026-04-24 per §2.9.1a — explicit Configuration vs Runtime Input axis named; post-freeze 2026-04-25 Q2 per §2.9.1b — three-axis distinction confirmed: trait-method-input axis vs trigger-purpose-input axis vs configuration-axis; **post-freeze 2026-04-25 Q3 per §2.9.1c** — four-axis distinction adds schema-as-data vs schema-as-trait-type carrier axis (n8n consumers live on the data axis, where Nebula's surface is universal). Under all four axes, consolidation cannot honestly resolve the divergence. Configuration goes through `&self` + `ActionMetadata::parameters` universally; trait-method input divergence is what consolidation would actually have to consolidate; trigger-purpose framing the user names is correct workflow nomenclature but lives at the lifecycle layer, not the trait method signature; and schema-as-data consumers are satisfied today by `ActionMetadata::with_schema` universally without trait-level Input/Output.)

#### §2.9.6 Rationale

The analysis surfaces a **four-axis shape mismatch** that consolidation cannot honestly resolve. Before the rationale: **the §2.9 axis at the trait-system layer is Method-Input/Output, not Configuration, not Trigger-purpose, not Schema-as-data.** Per §2.9.1a, configuration (per-instance settings — RSS url, Kafka channel) lives in `&self` struct fields with schema declared through `ActionMetadata::parameters` via `with_schema` (per `crates/action/src/metadata.rs:292`); this is universal across all 4 variants and orthogonal to consolidation. Per §2.9.1b, the user's standard-nomenclature framing (events ARE trigger output; configuration is the user-supplied input) is correct at the trigger-purpose layer but does not survive translation to the trait method signature — consolidation lives on the trait, where "Input/Output" name method parameters and method returns. Per §2.9.1c, schema-as-data consumers (UI generation, port-typing, validator filter rules — including n8n-parity reflection per the third post-freeze re-examination) are satisfied universally via `ActionMetadata.base.schema` / `inputs` / `outputs` regardless of trait variant; consolidation lives on the schema-as-trait-type axis (compile-time / type-system-mediated reflection) where no current consumer exists. The shapes below concern Method-Input — what the engine threads to `execute(.., input)` / `handle(.., event)` per dispatch — which is the axis consolidation would actually consolidate.
...
2. **Sub-trait pattern (Option B) has no current consumer on the schema-as-trait-type axis.** Per §2.9.1c, schema reflection has two carriers: the schema-as-data axis (runtime, JSON-reflected via `ActionMetadata.base.schema` / `inputs` / `outputs`) and the schema-as-trait-type axis (compile-time, monomorphized, type-system-mediated). All current consumers — UI form generation, downstream port-typing, validator filter rules, n8n-parity reflection (per the third post-freeze re-examination §2.9.1c) — live on the schema-as-data axis, which is universal across all 4 trait variants today and goal-aligned with [`docs/COMPETITIVE.md`](../../COMPETITIVE.md) line 41 typed-Rust-integration-contracts bet (not n8n surface parity). `ExecutableAction<I, O>` would be a new surface on the schema-as-trait-type axis, where the engine's `Arc<dyn StatelessHandler>` JSON-erasure path forecloses compile-time consumption (§2.4 / §2.5). Per `feedback_active_dev_mode.md`, speculative trait-type-axis surface area without a consumer that requires compile-time over runtime reflection is technical debt — adding it now means ADR-0038 / ADR-0039 must absorb it without a current beneficiary.
...
**N/A — REJECT (refined three times).** Status quo §2.2 signatures preserve verbatim. No refactor checklist. ADR-0038 ratification is unaffected; ADR-0035 §4.3 obligation is satisfied without change. Spike `final_shape_v2.rs:209-262` remains the signature-locking source. The Q2 post-freeze refinement (§2.9.1b) tightened the rationale by naming the three lifecycle axes explicitly (trait-method-input vs trigger-purpose-input vs configuration); the Q3 post-freeze refinement (§2.9.1c) added the fourth axis — schema-as-data vs schema-as-trait-type carrier — and acknowledged n8n consumer evidence (UI generation, port-typing, filter validation) as concrete and correct on the data axis where Nebula's surface is universal. The verdict is unchanged across all three iterations because none of the four axes enables consolidation.
...
**Storage shape.** Engine maintains a registry-time map `Map<ActionKey, &'static [SlotBinding]>` populated when `ActionRegistry::register*` is invoked. Per ADR-0039 §1 the macro emits the `&'static [SlotBinding]` slice as part of the `ActionSlots` impl (the `&self` receiver returns the same `&'static` slice for every action instance); engine's `register*` call sites live in `nebula-engine` (`crates/engine/src/registry.rs` is the current host per Phase 0 audit; exact line range and final host-crate path are CP3 §7 scope — `crates/runtime/` does not exist per Phase 1 workspace audit row 4). Engine iterates the slice once at registration and clones the binding entries (cheap — `SlotBinding: Copy`) into the registry-side index.
...
2. **Engine looks up** the `SlotBinding` for `self.bb`'s field via the registry-time index; the binding carries the macro-emitted `resolve_fn: ResolveFn` (per ADR-0039 §1 line 47-63).
...
**Builder method.** `ActionMetadata::with_max_concurrent(self, max: NonZeroU32) -> Self` — append to the existing builder chain at `crates/action/src/metadata.rs`. Macro-emission path (per ADR-0039 §1) reads `#[action(max_concurrent = N)]` attribute zone where present and threads through `with_max_concurrent(NonZeroU32::new(N).unwrap())`; emission is no-op when attribute absent.
...
This section locks the **production emission contract** for the `#[action]` attribute macro per [ADR-0039 §1](../../adr/0039-action-macro-emission.md). ADR-0039 names the load-bearing constraints (HRTB fn-pointer shape, dual enforcement layer, qualified-syntax Clone shadow probe, per-slot perf bound); §4 below is the implementer-grade contract that ADR-0039 ratifies.

The macro replaces `#[derive(Action)]` (current `crates/action/macros/src/derive.rs`-style emission per `crates/action/macros/src/action.rs:39-50`) with an attribute macro that participates in **field-zone rewriting** within the struct definition (per [ADR-0038 §Decision item 1](../../adr/0038-action-trait-shape.md)). Migration is hard-cut — `#[derive(Action)]` ceases to exist post-cascade per ADR-0038 §Negative item 1 + `feedback_hard_breaking_changes.md`. Codemod design lands at CP3 §9 per Strategy §4.3.3.

### §4.1 Attribute parser zones

The macro accepts the following zones per [ADR-0038 §Decision item 1](../../adr/0038-action-trait-shape.md) ("rewriting confined to fields declared inside `#[action(credentials(slot: Type), resources(slot: Type))]` attribute zones"):
...
Per [ADR-0038 §Decision item 1](../../adr/0038-action-trait-shape.md) — **rewriting is confined to fields declared inside the `credentials(...)` / `resources(...)` zones**. Fields outside the zones pass through unchanged.
...
**Why narrow.** ADR-0038 §Negative item 2 + `feedback_idiom_currency.md`: pervasive struct-level rewriting harms LSP / grep / IDE hover semantics ("why does this `String` field act like `&str`?" mysteries). Narrow zone-bounded rewriting keeps non-zone fields visible-meaning-preserved while opt-in zones gain typed-handle injection.
...
Per [ADR-0038 §Decision item 3](../../adr/0038-action-trait-shape.md) + [ADR-0039 §2](../../adr/0039-action-macro-emission.md). Both layers ship in production:
...
This is the **structural ground**: even if a malicious or buggy author bypasses the proc-macro layer (e.g., hand-implements `ActionSlots` on a bare-`CredentialRef` struct), they still hit the type system. ADR-0038 §Negative item 2 names this property as load-bearing.

#### §4.4.2 Proc-macro layer (DX, helpful diagnostic)

When the macro parses an `#[action]` invocation, it walks the struct body and detects any field whose type is `CredentialRef<_>` (or its dyn-shaped equivalents) that is NOT also declared in the `credentials(...)` zone. On such a field, the macro emits `compile_error!("did you forget to declare this credential in `credentials(slot: Type)`?")` with span pointing at the offending field. This fires **before** the type-system layer would error — cleaner DX per [ADR-0039 §2](../../adr/0039-action-macro-emission.md) bullet 2.
...
Both layers are intentionally redundant at the catch-the-bug level. The type-system layer is the structural truth; the proc-macro layer optimizes the diagnostic (per ADR-0038 §Negative item 2 — "removing either weakens the contract").
...
**Open item §4.4-1** — should the trait be sealed (per ADR-0035 §3 sealed convention) to prevent hand-implementation entirely? CP3 §9 considers; CP2 leaves `ActionSlots` `pub` because the macro is the recommended path and the spike + ADR-0039 §1 do not require seal.

### §4.5 Per-slot emission cost bound

Per [ADR-0039 §5](../../adr/0039-action-macro-emission.md) + spike §2.5 ([NOTES](../drafts/2026-04-24-nebula-action-redesign/07-spike-NOTES.md)):
...
**Open item §5.1-1** — `cargo-public-api` snapshot for the macro crate is **out of scope** per ADR-0039 §4 ("macro test harness ships with implementation"). Surface stability is at the trait level (§2), not the proc-macro internal token level. CP3 §9 may revisit if reviewer flags.
...
| 3 | Bare `CredentialRef<C>` field outside `credentials(...)` zone | `E0277` (type-system layer per §4.4.1) **AND** `compile_error!` (proc-macro layer per §4.4.2) | spike NOTES §1.4 + ADR-0039 §2 |
| 4 | `<SchemeGuard<'_, C> as Clone>::clone(&guard)` qualified-syntax probe — see §5.4 | `E0277` (`SchemeGuard: !Clone`) | spike NOTES §1.5 + ADR-0039 §3 |
...
Per [ADR-0039 §3](../../adr/0039-action-macro-emission.md) + spike finding #1 ([NOTES §3](../drafts/2026-04-24-nebula-action-redesign/07-spike-NOTES.md)):
...
**Production probe form (mandated by CP2 + ADR-0039 §3):**
...
This is the **silent-pass shape** flagged by spike finding #1. Per ADR-0039 §3 + ADR-0035 amended-in-place precedent, this is a **soft amendment candidate** к credential Tech Spec — the probe form should re-pin to:
...
1. **§5.4.1** — credential Tech Spec §16.1.1 probe #7 — qualified-syntax form `<SchemeGuard<'_, C> as Clone>::clone(&guard)` replaces naive `guard.clone()` to defeat auto-deref silent-pass (per ADR-0039 §3 + spike finding #1).
...
CP3 §7 closure (sealed-DX trait-by-trait audit per ADR-0040 §Implementation notes) is satisfied by the §2.6 R6 amendment + this §8.1.2 cursor-lifetime restoration.
...
The four primary dispatch traits (`StatelessAction` / `StatefulAction` / `TriggerAction` / `ResourceAction` per §2.2) remain `pub` at the trait level. Per [ADR-0038 §Neutral item 2](../../adr/0038-action-trait-shape.md) line 80: "Public API surface of the 4 dispatch traits is unchanged at the trait level — only the macro that constructs implementations changes shape."
...
### §9.2 Five sealed DX trait surface (sealed pattern per ADR-0040)

Five DX specialization traits — `ControlAction`, `PaginatedAction`, `BatchAction`, `WebhookAction`, `PollAction` — become sealed per [ADR-0040 §1](../../adr/0040-controlaction-seal-canon-revision.md) line 49-70. Trait identifiers remain `pub` (community plugin code that names them in trait bounds compiles); the **implementation surface** is sealed via per-capability inner sealed traits (per ADR-0035 §3 + ADR-0040 §1):
...
| `nebula_action_macros::Action` (derive) | `crates/action/src/lib.rs:91-153` | `nebula_action_macros::action` (attribute) | ADR-0038 §Decision item 1 (hard break per ADR-0038 §Negative item 1) |
...
- The five DX trait identifiers (`ControlAction`, `PaginatedAction`, `BatchAction`, `WebhookAction`, `PollAction`) — bound-shape only; community plugins do not impl directly per §9.2 / ADR-0040.
...
| **T6** | `impl ControlAction for X` → `impl StatelessAction for X` + `#[action(control_flow, ...)]` | ControlAction direct-impl call sites (community plugin migration target per ADR-0040 §Negative item 4) | StatelessAction primary + control-flow attribute zone (flag form per §12.2; CP4 §15 may revisit `control_flow = SomeStrategy` config form) | **MIXED** — **AUTO** for the trivial pass-through case (default mode); **MANUAL REVIEW** marker for control-flow-specific behavior (custom Continue/Skip/Retry reasons; Terminate interaction; test fixtures) | New at CP3; per ADR-0040 §Negative item 4 ("Codemod can cover the common case; edge cases (control-flow-specific behavior) need hand migration") — codemod attempts AUTO first, falls back to MANUAL marker on edge-case detection |
...
- **Mixed**: T6 (ControlAction migration) — AUTO for trivial pass-through (default mode); MANUAL marker on edge-case detection per §10.2.1. Per ADR-0040 §Negative item 4 ("common case auto / edge cases manual").
...
Per [ADR-0038 §Decision item 2](../../adr/0038-action-trait-shape.md) the macro emits:
...
For these cases, the **sealed_dx adapter pattern from [ADR-0040 §1](../../adr/0040-controlaction-seal-canon-revision.md)** is the contract:
...
This section locks the **ControlAction migration contract** per [ADR-0040](../../adr/0040-controlaction-seal-canon-revision.md). CP1 §2.6 already locked the sealed-DX trait family; CP3 §12 details the migration path from today's `pub trait ControlAction { ... }` (non-sealed, public-impl-allowed) to the sealed shape, and from the community plugin author perspective.

### §12.1 Sealed adapter pattern (verbatim from [ADR-0040 §1](../../adr/0040-controlaction-seal-canon-revision.md))

Per ADR-0040 §1 line 49-70 (verbatim):
...
Community plugin authors **do NOT implement any sealed DX trait directly**. Per [ADR-0040 §Negative item 4](../../adr/0040-controlaction-seal-canon-revision.md) line 111 (verbatim):
...
- **`crates/action/src/lib.rs:4`** library docstring becomes truthful per ADR-0040 §Neutral item 2 — currently self-contradicts "adding a trait requires canon revision" (line 4 verbatim) while re-exporting 10 traits; post-cascade it states the actual shape (4 primary + 5 sealed DX).
- **PRODUCT_CANON §3.5 line 82** — wording revision per [ADR-0040 §2](../../adr/0040-controlaction-seal-canon-revision.md). Inline canon edit lands as PR alongside ADR-0040 ratification per ADR-0040 Implementation note.

### §12.4 Codemod coverage (T6) — common-case automation

Per §10.2 transform T6 + ADR-0040 §Negative item 4 ("Codemod can cover the common case; edge cases need hand migration"):
...
- ADR-0039 §1 `SlotBinding` shape divergence (CP2 §15 forward-track) — `field_name` vs `key`, capability folded into `SlotType` enum vs separate field. Lands as `*Amended by ADR-0039 inline edit, 2026-04-24*` prefix at §1; CHANGELOG entry.
...
For breaking changes that **invalidate** the prior decision rather than refine it, full ADR supersession applies per ADR-0035 / ADR-0038 / ADR-0039 / ADR-0040 own status mechanics: `superseded` status set + `supersedes:` / `superseded_by:` cross-reference in frontmatter.

Examples (none active in current Tech Spec scope; all hypothetical):

- If a future Rust release adds `for<'ctx> async fn(...)` syntax (per [ADR-0039 §Negative item 1](../../adr/0039-action-macro-emission.md) line 121), the HRTB fn-pointer shape may be superseded by an `async fn` pointer shape. ADR-0039 supersession lands; macro emission shifts.
...
**Discipline.** Supersession pre-empts amendment-in-place when the original decision's name no longer fits the new shape — e.g., "ADR-0038 action-trait-shape" describes attribute-macro-replaces-derive; if a future cascade restores derives via a different mechanism, supersession (not amendment) is the honest record.
...
|---|---|---|---|
| **CR1** | Typed credential surface unrealized | §1 G1 + §3.1 + §4.1.1 + §11.1 | `#[action]` macro emits `&'static [SlotBinding]` per ADR-0039 §1 (post-amendment); spike Iter-1 PASS |
| **CR2** | Macro emission shape regressions | §1 G2 + §5 macro test harness + ADR-0039 §4 6-probe table | trybuild + macrotest dev-deps; 6 probes ported from spike commit `c8aef6a0` |
...
| **CR7** | Canon §3.5 governance debt (ControlAction not seal-ratified) | §1 G4 + §2.6 + ADR-0040 + §16.5 canon §3.5 revision PR | Sealed-DX trait family + canon line 82 revision |
...
This section walks every open item raised across CP1-CP3 to decided / deferred-with-trigger status. Carry-forward items get explicit closure rows; one cross-crate amendment is enacted in this CP (§15.5 ADR-0039 §1 SlotBinding shape per §0.2 invariant 2). Two soft amendments to credential Tech Spec are flagged (NOT enacted) per ADR-0035 amended-in-place precedent — credential Tech Spec author lands the inline edit during cross-section pass.
...
| §5.1-1 — `cargo-public-api` snapshot for macro crate | **DEFERRED-WITH-TRIGGER** | §15.8 — out of cascade scope per ADR-0039 §4; future macro-evolution housekeeping |
...
This CP **enacts** the ADR-0039 §1 amendment-in-place per ADR-0035 precedent. The enactment is recorded as a separate edit on `docs/adr/0039-action-macro-emission.md` co-landing with this CP4 draft:
...
- **Cross-section consistency.** ADR-0039 §3 Auto-deref Clone shadow probe section references `SlotBinding` shape only in the body — qualified-syntax probe form is preserved. ADR-0039 §4 Macro test harness section references the 6-probe table — `SlotBinding` shape not load-bearing for individual probe rows. ADR-0039 §5 Emission perf bound section refers to `LOC of macro-emitted region` — `SlotBinding` shape not load-bearing for naive-vs-adjusted ratio.
...
**Status invariant.** Per §0.2 invariant 2: ADR-0039 amendment is recorded as `proposed (amended-in-place 2026-04-25)`. This Tech Spec ratification (CP4 freeze) is conditional on ADR-0039 amendment landing — verified in §16.5 cascade-final precondition. ADR-0039 status moves to `accepted` upon Tech Spec ratification per §0.1 line 35.
...
| **(g)** ADR-0039 §1 SlotBinding shape amendment-in-place | ENACTED §15.5 (this CP) | architect | Action-cascade-internal — **ENACTED** |
...
- **ADR-0038** locks trait-shape (the family of action traits, the macro emission contract for action structs) — the `*Handler` per-method async return shape is **not** locked by ADR-0038. No ADR-0038 edit needed.
- **ADR-0039** locks macro emission for the action struct (`ActionSlots::credential_slots()` shape, `SlotBinding` shape, qualified-syntax probe, test harness, perf bound) — none of these are sensitive to whether `*Handler` uses `#[async_trait]` vs manual `BoxFut`. No ADR-0039 edit needed beyond the §15.5 amendment already enacted.
...
#### §15.9.6 Q3 post-freeze refinement record

Q3 (n8n trigger-node consumer evidence + trait-level vs method-level framing distinction) was re-examined post-freeze 2026-04-25 (third re-examination of §2.9 — see [`13-third-reanalysis-n8n-consumer.md`](../drafts/2026-04-24-nebula-action-redesign/13-third-reanalysis-n8n-consumer.md) for the full analysis chain, Q1-Q4 + outcome). Verdict: REJECT (refined three times). Refinement landed at:

- **§2.9.1c** (new sub-subsection) recording the user's third pushback verbatim with concrete consumer evidence (n8n's 70+ trigger nodes — Telegram / GitHub / Schedule examples with `INodeTypeDescription.properties` / `inputs` / `outputs` schemas) and naming the schema-as-data vs schema-as-trait-type carrier axis distinction explicitly. Q3 surfaced the legitimate fact that prior REJECTs framed "no current consumer" too narrowly without distinguishing carrier axes.
- **§2.9.5 decision header** extended to "(refined three times)" with four-axis decomposition.
- **§2.9.6 rationale prelude + point 2** updated: prelude names four axes (Method-Input/Output, Configuration, Trigger-purpose, Schema-as-data); point 2 refines "no current consumer" to "no current consumer **on the schema-as-trait-type axis**" — acknowledging schema-as-data consumers exist and are satisfied universally.
- **§2.9.7 Implications + Re-open trigger second bullet** updated: implications wording extended to four axes; re-open trigger bullet refined from "concrete consumer for typed Input/Output reflection" to "concrete schema-as-trait-type consumer" (compile-time / type-system-mediated walking by `Action<I, O>` type identity), with [`docs/COMPETITIVE.md`](../../COMPETITIVE.md) line 29-41 citation explicitly disclaiming n8n surface parity as a re-open trigger.

**No ADR amendment.** ADR-0038 / 0037 / 0038 unaffected — none specify trait-level Input/Output beyond what §2.2 already locks; this refinement does not touch §2.2 signatures or the macro emission contract. ADR-0035 phantom-shim composition is preserved (the `&self` configuration carrier is structurally load-bearing for §4.3 obligation; the user's `handle(.., input, event)` refactor proposal that would have broken this is REJECTED in §2.9.1c).

**No signature ripple.** Spike `final_shape_v2.rs:209-262` remains the signature-locking source. No `type Input` / `type Output` added to TriggerAction. The asymmetry between TriggerAction (no `Input`/`Output`) and Stateless/Stateful/Resource (each with `Input`/`Output`) is preserved as honest reflection of the dispatch-shape divergence (input-driven vs event-driven) per §2.9.2 + §2.9.6.

**No status qualifier change.** Per §15.9.5 precedent established for Q2: rationale-tightening amendments without signature ripple do not warrant a separate status header qualifier. Status header remains "amended-in-place 2026-04-25 — Q1 post-freeze" — Q1 is the only structural amendment (§2.3 + §2.4 `*Handler` shape change); Q2 + Q3 are rationale refinements absorbed into §2.9 sub-subsections + §15.9.5/§15.9.6 enactment records.

**Why amend-in-place vs supersede.** Per ADR-0035 §Status block "canonical-form correction" criterion + §15.9.2 / §15.9.5 precedent: rationale tightening that names a previously-implicit axis explicitly is canonical-form correction, not paradigm shift. Supersede would be disproportionate; amendment-in-place is the mechanism per `feedback_adr_revisable.md` precedent for ADR-0035 itself.

**Phase 8 cascade summary impact.** None. Q1 path pick (§16.1 a/b/c) is independent of §2.9 verdict. The user's Phase 8 framing remains as drafted in §16.1 — Q3 refinement does not surface new path options.
...
- **ADR-0038** locks trait-shape via §Decision items 1-4 (rewriting scope; emission contract; dual enforcement; ADR-0035 phantom composition). Per ADR-0038 §Neutral block last bullet: "Public API surface of the 4 dispatch traits ... unchanged at the trait level — only the macro that constructs implementations changes shape." This phrasing preserves the four-trait family enumeration; it does NOT lock per-method signatures verbatim. Tech Spec §2.2 is the per-method signature lock; spike `final_shape_v2.rs:209-262` is the signature-locking source per §0.1 inputs frozen-at footer. ADR-0038 status (`accepted` 2026-04-25) is preserved.
- **ADR-0039** locks macro emission for the action struct (`ActionSlots::credential_slots()` shape, `SlotBinding` shape, qualified-syntax probe, test harness, perf bound). Lifecycle methods are not in macro-emitted scope — implementors author them directly. ADR-0039 status (`proposed (amended-in-place 2026-04-25)` per §15.5) is preserved.
- **ADR-0040** locks sealed DX tier + canon §3.5 revision. Lifecycle methods are at the primary-trait layer, not the DX layer. ADR-0040 status (`proposed`) is preserved.
...
| **B4 — ADR-0038 binds verbatim spike shapes** | **STILL HOLDS.** Spike `final_shape_v2.rs:254-262` has no `type Input` on TriggerAction; bundle would invalidate freeze per §0.2 invariant 4 (spike-shape divergence trigger) without re-spike. |
...
**ADR composition.** ADR-0038 + ADR-0039 + ADR-0040 statuses preserved. ADR-0038 §Decision items 1-4 unchanged; lifecycle methods are at the per-method signature layer (Tech Spec §2.2 lock), not at trait-family enumeration layer (ADR-0038 §Neutral block lock). The four-ADR composition with ADR-0035 phantom-shim is unaffected.
...
**ADR composition.** ADR-0024 + ADR-0038 + ADR-0039 + ADR-0040 (4-ADR set, plus ADR-0035 phantom-shim governing field-zone rewriting) compose without conflict. None of the Q7 amendments touch ADR §Decision items.
...
- **ADR-0038** (action trait shape) — unaffected at the §Decision item layer. F2 amendment adds an opt-in default method to TriggerAction; ADR-0038 §Neutral block "Public API surface of the 4 dispatch traits ... unchanged at the trait level — only the macro that constructs implementations changes shape" preserves four-primary-trait-family enumeration; per-method amendments live at §2.2 lock per §15.11.1 / §15.10.1 precedent.
- **ADR-0039** (action macro emission) — unaffected. F9 `max_concurrent` field on ActionMetadata is metadata-builder-level; macro emission for the action struct is unchanged. ADR-0039 §1 SlotBinding shape preserved verbatim.
- **ADR-0040** (ControlAction seal + canon revision) — unaffected. F2 hook is on the primary `TriggerAction` trait, not on a sealed DX trait. ADR-0040 status (`proposed` pending user ratification on canon §3.5 revision) preserved per cascade prompt — Q8 enactment does NOT auto-flip ADR-0040.
...
**ADR composition.** ADR-0024 + ADR-0035 + ADR-0038 + ADR-0039 + ADR-0040 — all preserved per §15.12.1 per-ADR composition analysis. None of the Q8 amendments touch ADR §Decision items. Q8 enactment does NOT auto-flip ADR-0040 — pending explicit user ratification on canon §3.5 revision per cascade prompt.
...
| 5 | AgentAction + ActionTool cascade | NEW primary trait family (AI ≠ trigger / event); likely canon §3.5 revision per ADR-0040 §2 enumeration discipline | After action redesign implementation; AI use case priority |
...
- **Deliverable form.** `cargo`-style binary `nebula-action-codemod` (host: `tools/codemod/` placeholder per §10.2.1; exact crate name + binary location is §15.8 row (h)). AUTO mode default for T1/T3/T4; MANUAL-REVIEW marker for T2/T5; MIXED for T6 per ADR-0040 §Negative item 4.
...
**Plus implicit:** ADR-0038 / ADR-0039 / ADR-0040 status moves from `proposed` to `accepted` upon Tech Spec ratification (per §0.1 line 35); ADR-0039 has `proposed (amended-in-place 2026-04-25)` qualifier per §15.5 enactment.
...
- [ ] **ADR-0038 / ADR-0039 / ADR-0040 status moved to `accepted`** per §0.1 line 35 (status moves at Tech Spec ratification).
- [ ] **ADR-0039 §1 SlotBinding shape amendment-in-place landed.** Per §15.5.1 enactment — verified by `grep`-able anchor at ADR-0039 §Status block ("amended-in-place 2026-04-25") + §1 SlotBinding shape matching this Tech Spec §3.1 line 606-633.
- [ ] **Canon §3.5 revision PR ratified.** Per ADR-0040 §2 — canon §3.5 line 82 revises to enumerate the DX tier explicitly. This is a separate PR from cascade-landing; precondition for ADR-0040 status moving to `accepted`.
...
- §2.6 — DX trait blanket-impl trait-by-trait audit (which primary each DX wraps; ADR-0040 §Implementation notes "trait-by-trait audit at Tech Spec §7 design time") — CP3 §7 scope.
...
- §2.6 — added "Community plugin authoring path" paragraph naming the migration target (`StatelessAction` / `StatefulAction` + `#[action(...)]` attribute zones) per ADR-0040 §1 + §Negative item 4. Closes dx-tester R3 (blocking).
...
- §2.8 — added `SchemeGuard<'a, C>` non-Clone cross-ref + ADR-0039 §3 qualified-syntax probe discipline. Closes rust-senior 08b 🟠.
...
- §2 — Trait contract: `Action` base + 4 primary dispatch traits (Stateless/Stateful/Trigger/Resource) + `BoxFut<'a, T>` alias + 4 dyn-safe `*Handler` companions + 4-variant `ActionHandler` enum (no Control variant) + 5 sealed DX traits (per ADR-0040 §1 + ADR-0035 §3 sealed convention) + `ActionResult` with Terminate decision (wire-end-to-end picked per §2.7.1) + `ActionError` taxonomy preserved per rust-senior 02c §7 line 428.
...
- §5.1-1 — `cargo-public-api` snapshot for macro crate stability — CP3 §9 may revisit; out of scope for CP2 per ADR-0039 §4.
...
- **ADR-0039 §1 SlotBinding shape divergence — amendment-in-place trigger (rust-senior 09b #3).** ADR-0039 §1 currently shows `SlotBinding { key, slot_type, capability, resolve_fn }` with separate `capability` field; this Tech Spec §3.1 correctly folds capability into the `SlotType` enum per credential Tech Spec §9.4. Per ADR-0035 amended-in-place precedent, ADR-0039 §1 must be amended to mirror Tech Spec §3.1's `SlotBinding { field_name, slot_type, resolve_fn }` shape (capability lives inside `SlotType::ServiceCapability { capability, service }` and `SlotType::CapabilityOnly { capability }` variants). **FLAGGED, NOT ENACTED** — CP2 does not edit frozen ADRs (per task constraint); enactment is Phase 8 cross-section pass with ADR-0039 amended-in-place + CHANGELOG entry. This is a §0.2 invariant 2 trigger if not landed before Tech Spec ratification — Phase 8 must enact OR this Tech Spec must re-pin §3.1 to ADR-0039's current shape (rejected — credential Tech Spec §9.4 wins per cross-crate authoritative-source rule).
...
- CP3 §7 — `PollAction` sealed-DX trait shape lock per ADR-0040 §Implementation notes (cursor management; cluster-mode hooks).
...
- §15 — §5.3-1 marked RESOLVED; six items added during CP2 iteration (§2.9-1, §6.1.2-A/-B status, §6.3.1-A status, §4.1.3 cross-zone status, §5.4-companion status, ADR-0039 §1 amendment-in-place trigger); three items deferred from CP1 still un-homed (T4/T5/T9); three items forward-pointing CP4 cross-section.
...
- (b) §2.2.3 — TriggerAction cluster-mode hooks final trait shape per Strategy §5.1.5 line 297 — CP4 §15 scope (deferred from CP3 §7 per ADR-0040 trait-by-trait audit closure plus §1.2 N4 boundary).
...
- (g) ADR-0039 §1 SlotBinding shape divergence amendment-in-place — CP2 §15 forward-track preserved; Phase 8 enacts inline ADR edit + CHANGELOG entry. Per §0.2 invariant 2, must land before Tech Spec ratification.
...
- §15 (open items) — CP3 closures recorded (§2.7-2 partial / §3.2-1 partial / §4.4-1 / §6.2-1 / §6.5 / §10 transforms / §11.3 / §13.4 T4+T9); CP3-new items added (§9.3-1 / §11.3-1 / §12 attribute syntax); 10-item CP4 forward-track including ADR-0039 §1 amendment-in-place trigger preserved from CP2.
...
- §10.2 T6 row — verdict normalized to **MIXED** (AUTO default for trivial pass-through; MANUAL marker on edge-case detection) per ADR-0040 §Negative item 4. Closes spec-auditor 10a 🟠 HIGH on §10.2 vs §10.5 internal contradiction. §10.2.1 default-mode line revised to enumerate the MIXED behaviour explicitly.
...
- (g) ADR-0039 §1 SlotBinding shape amendment-in-place — **ENACTED** at §15.5.1 (ADR file edit lands co-with this CP4 draft)
...
- 🔴 #2 (supersede-stale §9.4) — §14.3 row 7 + row 8 re-pinned to `§15.8 (CP5 supersession of §9.4)` with explicit shape-preservation note (matching axes preserved; capability authority shifts plugin-metadata `capabilities_enabled` → registry-computed `RegistryEntry::capabilities`). §15.5 + §15.5.1 add supersession-acknowledgement paragraph and update §15.5.1 enactment-bullet citation. §3.1 SlotType `ServiceCapability` doc comment (lines 624-628) re-pinned: removes `cred.metadata().capabilities_enabled.contains(*capability)` reference; cites §15.8 + `RegistryEntry::capabilities`. ADR-0039 line 86 doc comment likewise re-pinned (separate Edit on ADR file; ADR amendment-in-place qualifier preserved). Closes spec-auditor 11a 🔴 #2.
...
- **spec-auditor** — please **full cross-CP audit** before freeze: (a) cross-section consistency — every forward reference resolves to actual content (especially §14.1 ADR matrix → all four ADRs at correct status; §14.2 / §14.3 / §14.4 / §14.5 cross-refs all `grep`-able at cited line ranges); (b) every claim grounded in code (file:line) / canon / ADR / Strategy / Phase 0 audit / spike artefacts at line-number granularity; (c) §15.5.1 ADR-0039 amendment-in-place actually enacted in `docs/adr/0039-action-macro-emission.md` (verify status header + §1 SlotBinding shape + CHANGELOG entry); (d) §15.3 + §15.4 soft amendments are FLAGGED only (no inline credential Tech Spec edit performed by this Tech Spec); (e) terminology alignment with `docs/GLOSSARY.md`; (f) §16.1 (a)/(b)/(c) framing aligns with Strategy §4.2 + §6.5 verbatim shape; (g) §16.5 pre-implementation checklist captures every cascade-final precondition (Tech Spec ratified + 4 ADRs accepted + ADR-0039 amendment-in-place + canon §3.5 revision PR + cred CP6 cascade slot + soft amendments coordinated + concerns register clean); (h) line-count budget — CP4 §14-§16 within 200-300 line target.
- **tech-lead** — please ratify §15 closures (especially §15.5 ADR-0039 amendment-in-place enactment) and §16 framing (especially §16.3 7-item DoD checklist as the cascade-landing PR-wave-level definition of done, and §16.4 rollback strategy layer split). Solo-decider authority on §15.5 enactment (cascade-internal cross-cutting); CP4 freeze requires tech-lead explicit ratification. **Note:** §16.1 user-facing path framing is presented at Phase 8 cascade summary — Tech Spec presents, user picks; tech-lead ratifies the framing shape, not the user's pick.
...
- §15.9 (NEW subsection) — Q1 post-freeze amendment-in-place enactment recorded. §15.9.1 enactment table (no ADR file edit needed — ADR-0024 is source-of-truth and unchanged; ADR-0038/0037 do not lock `*Handler` async-fn shape; only Tech Spec inline edits at §2.3 + §2.4). §15.9.2 amend-in-place vs supersede rationale per ADR-0035 §Status block "canonical-form correction" criterion. §15.9.3 cross-cascade impact (Cargo.toml `async-trait` dep add; no `crates/action/src/handler.rs` ABI change; mechanical migration on stabilization). §15.9.4 §16.5 cascade-final precondition unchanged. §15.9.5 records Q2 refinement as rationale-tightening only (no separate Q2 amendment-in-place qualifier).
...
- ADR transitions: NONE. ADR-0024 + ADR-0038 + ADR-0039 + ADR-0040 statuses preserved. ADR-0024 was already accepted 2026-04-20 and remains so; this amendment aligns Tech Spec with ADR-0024's pre-existing decision.
...
**Cascade-state changes:**
- ADR transitions: NONE. ADR-0024 / ADR-0038 / ADR-0039 / ADR-0040 statuses preserved. ADR-0024 (workspace policy on `*Handler` `#[async_trait]` adoption) is unaffected — Q7 R4/R5 amend the per-method parameter types of TriggerHandler/ResourceHandler, not the async-fn-in-trait shape. ADR-0038 §Decision items 1-4 unchanged (items lock trait-shape rewriting / emission contract / dual enforcement / phantom composition — not per-method signatures). ADR-0038 §Neutral block "Public API surface of the 4 dispatch traits ... unchanged at the trait level" preserves four-primary-trait family enumeration; per-method amendments live at §2.2 lock. ADR-0040 §1 sealed-pattern intent preserved — seal still prevents external `impl WebhookAction for X`; only bound chain re-pins to peer-of-Action.
...
**Cascade-state changes:**
- ADR transitions: NONE. ADR-0024 + ADR-0035 + ADR-0038 + ADR-0039 + ADR-0040 statuses preserved. ADR-0040 status (`proposed`) preserved per cascade prompt — Q8 enactment does NOT auto-flip ADR-0040 (still pending explicit user ratification on canon §3.5 revision; surface to user in Phase 8 summary per cascade prompt).
