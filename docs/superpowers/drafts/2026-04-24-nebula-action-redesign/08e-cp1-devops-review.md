...
- §2.1 `Action: ActionSlots + Send + Sync + 'static` supertrait — does not constrain dyn-safety of macro-emitted impls; the macro emits `impl ActionSlots for X` + `impl StatelessAction for X` + a blanket `impl Action for X` (per ADR-0037 §2 line 67-70). Compile-fail tests can target rejection paths (`credential = "string"`, non-unit struct, missing `Source`/`Resource`) without hitting dyn-safety at all.
