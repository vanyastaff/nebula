...
**Atomicity mapping (VERIFIED against `release_entry` `pool.rs:1175-1239` — this is the wall the first attempt flagged BLOCKED; it is designable, not a true blocker).** Do NOT delete `PoolRuntime`'s idle queue until these hold:
- Today's revoke fence atomicity is the `RecycleDecision::Keep` arm (`pool.rs:1221-1235`): `recycle().await` → **lock idle → re-read revoke epoch UNDER the lock → compare → `push_back` UNDER the same lock** — one continuous critical section, atomic against the revoke idle-walk (which takes the same lock).
- `InstanceStore::return_slot` ALREADY implements that exact critical section (lock → re-read epoch under lock → compare → push under the same lock). So the re-seat preserves atomicity **iff the pipeline runs `recycle()` + the pre-recycle pool-policy checks (tainted/fingerprint/max-lifetime/broken) FIRST, then `store.return_slot(slot, checkout_epoch)` LAST**. The pre-recycle early-out revoke check (`pool.rs:1205`) is an optimization (destroy before an expensive `recycle()`), replicate it as a `store.current_revoke_epoch()` compare before `recycle()` — `return_slot` is the authoritative fence, this is not correctness-load-bearing.
- Rotation: `dispatch_slot_hook_over_idle` holds `store.lock_idle()` across **every** hook `.await`, never drop-and-reacquire between entries (`pool.rs:330` warning). Same lock `checkout`/`return_slot` take → no checkout can interleave mid-rotation.
- **Proof obligation (the TOCTOU harness already exists):** `revoke_recycle_toctou::*`, `resident_rotation_race::*`, `shutdown_race::*`, and the HikariCP-#1836 in-flight-create fence stay green with UNCHANGED intent. Their green IS the atomicity proof — never weaken an assertion to pass.
...
- [x] **Step G2: A11 — cost-aware `check()` scheduling contract.** `check_cost()` (Phase D3) is consumed by the maintenance reaper to space expensive probes (Cheap ~10s, Expensive ~minutes). Test: a resource with `CheckCost::Expensive` is probed less often than a `Cheap` one over a fixed window (use `tokio::test(start_paused = true)`).
...
- [x] **Step H1: ADR for canon §11.4 revision.** Write `docs/adr/00NN-resource-release-is-async-work.md` (next number; check `docs/adr/README.md`): release/destroy/check are fallible async ordered work, not best-effort `Drop`; the in-process manager reconciles orphaned slots via the reaper; guaranteed-destroy for money/hardware is out-of-scope (external janitor, later spec). Update `docs/PRODUCT_CANON.md` §11.4 wording + add the ADR to `docs/adr/README.md`.
- [x] **Step H2: ADR (or fold into H1) for the async-trait policy.** Record the deliberate override: `#[async_trait]` crate-wide for nebula-resource because lifecycle is I/O-bound, async-trait is trusted/ubiquitous, migration-off is trivial, dynosaur is a heavier bet. Reference §2.7.
- [x] **Step H3: Mirror into the path-scoped standard + memory.** Update the project's Rust standard note that says "prefer native async-fn-in-trait" to carve out the nebula-resource I/O-bound + always-erased exception. Update the `feedback_idiom_currency` auto-memory (`C:\Users\vanya\.claude\projects\…\memory\feedback_idiom_currency.md`) with a one-line note: "nebula-resource deliberately uses #[async_trait] (I/O-bound, dyn-everywhere, trivial migration) — see ADR-00NN; do not 'modernize' it back."
- [x] **Step H4: Docs sweep.** Update `crates/resource/README.md`, `crates/resource/CLAUDE.md`, `crates/resource/docs/*` to the final vocabulary (Provider/Instance/Topology open trait/ManagedHandle/Bounded-restored/admission surface). Rustdoc gate: `RUSTDOCFLAGS="-D warnings" cargo doc -p nebula-resource --no-deps`.
- [x] **Step H5: Commit.**
```bash
