...
//!
//! Fields whose schema `Field` is `Field::Secret` are excluded **by schema
//! type** — pre-resolve a secret is `FieldValue::Literal(plaintext)`, so a
//! runtime-tag check would leak it into predicate evaluation and into the
//! validator-produced report.
...
/// Build a `PredicateContext` from top-level values, excluding any field
/// declared as `Field::Secret` in `fields`.
///
/// Only `FieldValue::Literal` values are addressable by predicates;
/// expressions/objects/lists/secret-sentinels are non-addressable, matching
/// the prior `RootContext` semantics for the literal case.
#[must_use]
pub fn predicate_context_for(fields: &[Field], values: &FieldValues) -> PredicateContext {
    ...
    for field in fields {
        if matches!(field, Field::Secret(_)) {
            continue; // never expose secret-typed fields to predicates
        }
        let key = field.key();
        if let Some(FieldValue::Literal(v)) = values.get(key) {
            pairs.push((FieldPath::single(key.as_str()), v.clone()));
        }
    }
...
> Implementer: verify `Field::secret(FieldKey)` exists as a constructor
> (`crates/schema/src/field.rs`; `Field::string`/`Field::secret` are used in
> `crates/schema/src/lint.rs:1472`). Verify `field.key() -> &FieldKey` and
> `FieldKey::as_str()` (used at old `context.rs:20-22`). `FieldPath::single`
> exists (`crates/validator/src/rule/context.rs:66`).
...
- secret scrub #1 (by `Field::Secret` schema type, pre-resolve plaintext) → Task 6 + lint Task 9. ✓
