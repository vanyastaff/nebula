...
- **[L2]** **Recovery preserves operation meaning:** refresh and revoke retain distinct durable operation kinds before provider dispatch. A refresh decision never releases an unresolved revoke. Confirmed revoke recovery atomically tombstones the pinned material authority, records its decision, and resolves its claim. New credential projections check outstanding operation authority even when the material is unexpired or has no refresh capability. A projection that finds a refresh crossing the provider boundary joins it for a bounded wait and serves the refreshed material once it commits, so a refresh is not an availability outage; revoke, reconciliation and reauthorization are never joined. Reconciliation names the incident it resolves, so a retried decision can never resolve a newer incident on the same credential. Historical incidents without a recorded operation remain unclassified and fail closed; migrations never infer their provider effect.
