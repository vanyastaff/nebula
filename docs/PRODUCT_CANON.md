...
- **Schema** — the cross-cutting typed configuration system (`nebula-schema`: `Property`, `Schema`, `ValidValues`, `ResolvedValues` with proof-token pipeline). Shared across Actions, Credentials, Resources.
...
- **[L2]** **Explicit failure on irreconcilable state:** a definitive provider rejection or missing refresh material transitions the credential to explicit `ReauthRequired`, surfaced in credential status and typed API errors. An ambiguous post-provider outcome is never mislabeled as success: its expired `RefreshInFlight` claim remains durable fail-closed poison and provider replay stays forbidden until authorized reconciliation. Repeated owner-qualified sentinel incidents inside the configured rolling window atomically install `ReauthRequired` while retaining that poison; audit and event-bus messages emitted afterward are observations of the committed aggregate state.
...
- **[L2]** **Recovery preserves operation meaning:** refresh and revoke retain distinct durable operation kinds before provider dispatch. A refresh decision never releases an unresolved revoke. Confirmed revoke recovery atomically tombstones the pinned material authority, records its decision, and resolves its claim. New credential projections check outstanding operation authority even when the material is unexpired or has no refresh capability. A projection that finds a refresh crossing the provider boundary (through the resolver or a resource or action slot) joins it for a bounded wait and serves the refreshed material once it commits, and reports busy rather than blocked if it does not, so a refresh is not an availability outage; revoke, reconciliation and reauthorization are never joined. Reconciliation names the incident it resolves, so a retried decision can never resolve a newer incident on the same credential. Historical incidents without a recorded operation remain unclassified and fail closed; migrations never infer their provider effect.
