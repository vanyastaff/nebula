...
| `nebula_credential_refresh_coord_sentinel_events_total` | counter | `action={recorded,reauth_triggered}` | 2 series | Sentinel accounting by the reclaim sweep. `recorded` ticks once per distinct poisoned claim UUID; repeated requests/sweeps do not spend the threshold budget. `reauth_triggered` ticks after the owner-qualified storage transaction commits the database-clock threshold and durable `ReauthRequired` aggregate transition. Event publication remains a lossy post-commit observation. |
