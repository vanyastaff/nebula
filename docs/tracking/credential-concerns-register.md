...
|---|---|---|---|---|
| draft-f1 | `ctx.credential::<C>()` ambiguity on multiple slots of same type | strategy-blocking | in-implementation | Spike Q2 validated hand-expanded macro output; П1 Stage 4 landed `#[action]` attribute + phantom-shim rewrite (commit `e0e324bf`) — Pattern 2 multi-slot ambiguity resolved structurally via `CredentialRef<dyn ServiceCapPhantom>`. |
...
| draft-f3 | `dyn Credential` / `dyn BitbucketCredential` not dyn-safe (4 assoc types) | strategy-blocking | decided | Strategy §3.2 — type-erased runtime path via `AnyCredential` + downcast; `dyn BitbucketBearer` is nominal bound only. П1 Stage 4 probe `compile_fail_dyn_credential_const_key` confirms structural block (E0038 from `const KEY`) — green at Stage 8 landing gate. |
| draft-f4 | `CredentialRef<C>` runtime shape — PhantomData only | strategy-blocking | locked-post-spike | Spike Q3 three hypotheses H1/H2/H3 (Strategy §3.4). Production type still post-П1 (Stage 4 fixture uses local stand-in per documented rationale). |
...
| draft-f14 | Multi-credential resource (mTLS + Bearer in one HTTP client) | strategy-blocking | locked-post-spike | Spike Q4 — DualAuth<A, B>. Post-П1 (consumer-side Pattern 2 wiring is П2+). |
| draft-f32 | `CredentialGuard<C::Scheme>` projection through dyn service trait | strategy-blocking | in-implementation | Spike Q1 validated pure marker + blanket sub-trait pattern (Strategy §3.3); П1 Stage 6 lands `SchemeGuard<'a, C>` + `SchemeFactory<C>` (§15.7 — closes N8). |
...
| arch-capability-subtrait-split | Replace 4 capability bools (`INTERACTIVE`/`REFRESHABLE`/`REVOCABLE`/`TESTABLE`) + production `DYNAMIC` with sub-traits `Interactive`/`Refreshable`/`Revocable`/`Testable`/`Dynamic`. `Pending` assoc type moves under `Interactive`. Engine dispatchers bind `where C: Refreshable`. Closes silent-downgrade vector when const says `true` but method defaults to `NotSupported`. (Closes security-lead N1+N3+N5.) | tech-spec-material | in-implementation | Tech Spec [§15.4](../superpowers/specs/2026-04-24-credential-tech-spec.md). П1 Stage 3 landed (worktree `worktree-credential-p1` — merge commit pending). Probes: `compile_fail_capability_subtrait_*` (4 probes — refreshable / revocable / testable / dynamic each missing-method) + `compile_fail_engine_dispatch_capability` all green at Stage 8 landing gate. Surfaced in 3-stakeholder consensus session 2026-04-24. |
...
| arch-registry-duplicate-fail-closed | `CredentialRegistry::register<C>` returns `Result<(), DuplicateKey>` fatal in BOTH debug + release. Replaces current "panic in debug, warn + overwrite in release" pattern (§3.1 line 663). Silent credential takeover via supply-chain plugin / namespace collision blocked at startup. (Closes security-lead N7 — interim, until signing-infra lands.) | tech-spec-material | in-implementation | Tech Spec [§15.6](../superpowers/specs/2026-04-24-credential-tech-spec.md). П1 Stage 5 landed (commit `c44eb2ca`; final landing-gate `runtime_duplicate_key_fatal` green at Stage 8). Snippet alignment §15.6 candidate (a) reconciled with landed code at Stage 8 (see `stage5-followup-s3`). 3-stakeholder consensus session 2026-04-24. |
| stage5-followup-i1 | `CredentialRegistry::register` visibility — Tech Spec §3.1 line 669 specifies `pub-crate` (only plugin init paths can call), code lands as `pub`. **Resolved 2026-04-26 Stage 8: keep `pub`.** Rationale: `CredentialRegistry` is held privately by the engine entry-point (`engine::new()` constructs it; no external mutable handle exists at runtime). Only plugin init paths invoke `register`, so the textual `pub-crate` constraint from §3.1 expresses intent rather than a security boundary. Forcing `pub(crate)` would require a Builder facade purely for visibility hygiene. Tech Spec §3.1 line 669 supersession to follow during the next §3 inline sync (tracked under `arch-techspec-section-sync`). | process | decided | Stage 8 doc-sync resolution. Code-quality review on `c44eb2ca`. Tech Spec [§3.1](../superpowers/specs/2026-04-24-credential-tech-spec.md) line 669. |
| stage5-followup-i2 | `CredentialKey(Arc<str>)` newtype — Tech Spec §3.2 specifies a `CredentialKey(Arc<str>)` newtype around the registry storage key, code uses raw `Arc<str>` directly in `AHashMap<Arc<str>, RegistryEntry>`. **Resolved 2026-04-26 Stage 8: keep raw `Arc<str>` for П1; revisit post-MVP if a non-`KEY` `Arc<str>` ever flows into the storage map.** Rationale: only `C::KEY` (a `&'static str` from the trait) is converted via `key.into()` at the single call site inside `register`; entry-point control rules out wrong-type insertion. The newtype would add a layer of `pub` surface for zero behavioural difference; the §3.2 spec text will be amended during the next inline sync. | process | decided | Stage 8 doc-sync resolution. Code-quality review on `c44eb2ca`. Tech Spec [§3.2](../superpowers/specs/2026-04-24-credential-tech-spec.md). |
| stage5-followup-s1 | `CredentialRegistry::keys()` iterator — Stage 7 (`iter_compatible` filter via registry-computed capabilities, §15.8) requires iterating registered credentials. **Resolved 2026-04-26 Stage 8: superseded by `iter_compatible`.** Stage 7 (commit `ecb999b3`) shipped `iter_compatible(required: Capabilities) -> impl Iterator<Item = (&str, Capabilities)>` directly, which subsumes the proposed `keys()` method (call with `Capabilities::empty()` to iterate all entries). No additional method needed. | process | decided | Stage 7 implementation. Code-quality review on `c44eb2ca`. Tech Spec [§15.8](../superpowers/specs/2026-04-24-credential-tech-spec.md). |
| stage5-followup-s2 | `register_with_capabilities` test helper — Stage 7 unit tests need to exercise capability-aware code paths without standing up real `Credential` impls with the full sub-trait surface. **Resolved 2026-04-26 Stage 8: not needed for П1.** Stage 7 unit tests stand up real (lightweight) `Credential` impls and rely on `compute_capabilities::<C>()` + sub-trait membership rather than a backdoor capability injection. A test helper that bypasses the type-system fold would weaken the §15.8 invariant ("capability set comes from the type, not from input") in the very probe surface that proves it. Defer indefinitely; if a Stage-N capability needs targeted probing, prefer adding a minimal real `Credential` impl. | process | decided | Stage 8 doc-sync resolution. Code-quality review on `c44eb2ca`. |
| stage5-followup-s3 | Tech Spec §15.6 snippet update — the `register` signature that landed in `c44eb2ca` differs from the §15.6 candidate (a) snippet (lines 3322-3360): (1) takes `registering_crate: &'static str` parameter explicitly instead of using `env!("CARGO_CRATE_NAME")` inside the function body (which would always resolve to `nebula-credential`, defeating the per-plugin attribution); (2) no `CredentialMetadataSource` bound on `C` (Stage 5 sticks to `C: Credential` — metadata-source bound is a Stage 7+ concern); (3) bound is just `C: Credential`. **Resolved 2026-04-26 Stage 8: §15.6 candidate (a) snippet updated to match landed signature** (5 `plugin_capability_report::Is*` bounds + explicit `registering_crate` parameter + `compute_capabilities::<C>()` fold + `RegistryEntry` storage). Snippet alignment note added inline. | process | decided | Stage 8 doc-sync resolution (this commit). Tech Spec [§15.6](../superpowers/specs/2026-04-24-credential-tech-spec.md) lines 3322-3360. |
| stage7-followup-engine-discovery | Engine `iter_compatible` consumer wiring — Stage 7 (commit `ecb999b3`) shipped `CredentialRegistry::iter_compatible` (Tech Spec §15.8) plus the `Capabilities` bitflag set + `compute_capabilities::<C>()` fold. The engine slot picker / discovery code (`crates/engine/src/credential/registry.rs` shim) now sees the producer surface but does not yet route slot-picker queries through it — the existing engine registry still uses kind-keyed lookups for compat with П1's pre-§15.8 wiring. Follow-up cascade swaps the slot picker to consume `iter_compatible` so Pattern 3 (`SlotType::CapabilityOnly`) actually filters by registry-computed capabilities rather than by self-attested metadata. Severity: low — `metadata.capabilities_enabled` is removed structurally (Probe 8 verifies E0609), so plugins cannot lie even before the slot-picker swap; the swap is a follow-up to wire the consumer side. | tech-spec-material | proposed | Post-П1 follow-up cascade. Stage 7 review (`ecb999b3`). Tech Spec [§15.8](../superpowers/specs/2026-04-24-credential-tech-spec.md). |
...
| draft-f17 | Mid-refresh crash with rotated refresh_token (IdP invalidates old) | sub-spec | done | П2 landed at `<MERGE_SHA>` (2026-04-26) — two-tier coordinator (L1 + L2 claim repo per ADR-0041) + sentinel N=3-in-1h threshold + `ReauthRequired` escalation; closes n8n #13088 class race. Spec [`docs/superpowers/specs/2026-04-24-credential-refresh-coordination.md`](../superpowers/specs/2026-04-24-credential-refresh-coordination.md) frozen at P2 ship; plan [`docs/superpowers/plans/2026-04-26-credential-p2-refresh-coordination-l2.md`](../superpowers/plans/2026-04-26-credential-p2-refresh-coordination-l2.md); ADR [`docs/adr/0041-durable-credential-refresh-claim-repo.md`](../adr/0041-durable-credential-refresh-claim-repo.md) |
...
| arch-scheme-sensitivity-dichotomy | Split `AuthScheme` → `SensitiveScheme: AuthScheme + ZeroizeOnDrop` + `PublicScheme: AuthScheme`. Removes "non-sensitive scheme carve-out" (§2.2 line 312) which was the rationale loophole for `WebhookUrlScheme` and similar URL-shaped secret bearers. Derive macros `#[auth_scheme(sensitive)]` / `#[auth_scheme(public)]` audit fields at expansion (forbid plain `String` for sensitive, forbid `SecretString` for public, name-based lint on `token`/`secret`/`key`/`password`). `OAuth2Token::bearer_header` returns `SecretString`; `ConnectionUri` exposes structured accessors. (Closes security-lead N2+N4+N10.) | tech-spec-material | in-implementation | Tech Spec [§15.5](../superpowers/specs/2026-04-24-credential-tech-spec.md). П1 Stage 1 landed (worktree `worktree-credential-p1` — merge commit pending). Probes: `compile_fail_scheme_sensitivity_*` (4 probes — plain String / public-with-Secret / public-with-Option-Secret / sensitive-no-zeroize) all green at Stage 8 landing gate. Macro field-name + field-type audit verified across 5 schemes. 3-stakeholder consensus session 2026-04-24. |
| arch-scheme-guard-factory | `Resource::on_credential_refresh` takes owned `SchemeGuard<'_, C>` (`!Clone + ZeroizeOnDrop`, lifetime-bound to call) instead of `&Scheme`. `SchemeFactory<C>` companion provides re-acquisition pattern for long-lived resources (HTTP connection pools, etc.) — resource never retains the Scheme. (Closes security-lead N8 + tech-lead technical gap (i).) | tech-spec-material | in-implementation | Tech Spec [§15.7](../superpowers/specs/2026-04-24-credential-tech-spec.md). П1 Stage 6 landed (commits `c25fc6ff` + `eae6e1e7` + `5e5ccfd0`). Probes: `compile_fail_scheme_guard_retention` + `compile_fail_scheme_guard_clone` both green at Stage 8 landing gate. `OnCredentialRefresh<C>` parallel trait shape per Tech Spec §15.7 spike iter-3 ratification. Resource-manager integration tracked under `stage6-followup-resource-integration`. 3-stakeholder consensus session 2026-04-24. |
| arch-publicscheme-nested-sensitive-audit | `#[derive(AuthScheme)]` `audit_fields` only rejects literal `SecretString` / `SecretBytes` (and their `Option` / `Box` / `Arc` / `Rc` wrappers) on `#[auth_scheme(public)]` schemes — it cannot detect a `public` scheme that embeds a nested `SensitiveScheme` (`SecretToken`, `OAuth2Token`, user-defined sensitive types, renamed re-exports, type aliases) because proc-macros only see their own input's tokens, not other crates' `impl SensitiveScheme for X` declarations. The trait-level `SensitiveScheme: AuthScheme + ZeroizeOnDrop` bound is defense-in-depth (forces author to derive ZeroizeOnDrop on the wrapping struct, which signals embedding sensitive material), but is not a strict negative-impl mechanism. PR #582 review surface 2026-04-26 (Copilot + CodeRabbit). | tech-spec-material | open | Doc-only mitigation landed in `crates/credential/macros/src/auth_scheme.rs` (`audit_fields` rustdoc) + `crates/core/src/auth.rs` (sensitivity-dichotomy enforcement note). Long-term: investigate compile-time reflection (e.g. associated `Sensitivity` tag via sealed-trait pattern, or `where Self::FieldsX: PublicScheme` accumulator) to reject nested-sensitive at the macro-emit site. Coordinate with `arch-signing-infra` since signed manifests would also let a registry-level audit reject `PublicScheme` impls that observably embed sensitive material at runtime. |
...
| arch-metadata-capability-authority | `iter_compatible` Pattern 3 (`SlotType::CapabilityOnly`) trusts plugin-declared `metadata.capabilities_enabled` — plugin can self-attest false capabilities to appear in slot pickers it shouldn't satisfy. Resolution: compute `capabilities_enabled` at registration time from `C`'s sub-trait membership (post §15.4 split), not from plugin metadata. `CredentialMetadata::capabilities_enabled` field removed. Plugin cannot lie. (Closes security-lead N6.) | tech-spec-material | in-implementation | Tech Spec [§15.8](../superpowers/specs/2026-04-24-credential-tech-spec.md). П1 Stage 7 landed (commit `ecb999b3`). Probe: `compile_fail_metadata_capability_field` green at Stage 8 landing gate (E0609 on absent `capabilities_enabled`). Engine slot-picker consumer wiring tracked under `stage7-followup-engine-discovery`. 3-stakeholder consensus session 2026-04-24. |
...
## Current totals (audited 2026-04-26 — Stage 8 added 1 tech-spec-material row stage7-followup-engine-discovery; flipped 5 architectural rows to in-implementation; resolved 5 stage5-followup process rows)

| Label | Count | Notes |
|---|---|---|
| strategy-blocking | 12 | All resolved in Strategy §2/§3 or locked-post-spike. П1 Stage 8: 2 flipped to `in-implementation` (draft-f1, draft-f32). |
| tech-spec-material | 92 | Most `locked-post-spike`; unlock with Tech Spec. CP5 added 6: arch-capability-subtrait-split, arch-registry-duplicate-fail-closed, arch-scheme-sensitivity-dichotomy, arch-scheme-guard-factory, arch-metadata-capability-authority, runtime-pending-consume-atomicity. CP6 post-review added 1: arch-subtrait-phantom-compose-risk. П1 Stage 6 review added 1: stage6-followup-resource-integration. П1 Stage 8 added 1: stage7-followup-engine-discovery. П1 Stage 8 flipped 5 architectural rows to `in-implementation` (arch-capability-subtrait-split, arch-registry-duplicate-fail-closed, arch-scheme-sensitivity-dichotomy, arch-scheme-guard-factory, arch-metadata-capability-authority). |
...
| process | 19 | Findings about the redesign workstream itself; CP5 added 1 (tech-spec-adoption-status, flipped CP6); CP6 added 3 (gate-p10-landing, gate-n7-registry-observability, gate-spike-iter3-dyn-safety); CP6 post-review added 2 (arch-cp5-spike-validation, arch-techspec-section-sync); П1 Stage 5 review added 5 (stage5-followup-i1..s3); П1 Stage 8 resolved all 5 stage5-followup rows to `decided` (i1+i2+s1+s2+s3). |
| **Total** | **150** | Counts audited at each register revision |
