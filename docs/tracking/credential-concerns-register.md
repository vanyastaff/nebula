...
| arch-subtrait-phantom-compose-risk | Open structural risk (surfaced by user review 2026-04-24 post-CP6). Sub-trait capability split (§15.4) + phantom-shim pattern (ADR-0035) have never been validated together — spike iter-1/iter-2 only covered CP4 shape (const bools). Unknowns: (a) `dyn Credential` object-safety preserved when Interactive/Refreshable/etc split; (b) does `dyn Refreshable` require a parallel `RefreshablePhantom` shim for runtime dispatch; (c) SchemeGuard<'a, C> lifetime × phantom composition; (d) `#[plugin_credential]` macro emission for sub-trait impl blankets. Gate 3 spike closes. | tech-spec-material | open | Closes via Gate 3 — `gate-spike-iter3-dyn-safety` row. Discovery risk that tech-lead Round 4 condition 1 explicitly flagged («don't discover it in П1»). |
| arch-cp5-spike-validation | Process tracking — CP5/CP6 shape (sub-trait split + SchemeGuard/SchemeFactory + sensitivity dichotomy + fatal duplicate-KEY + capability-from-type + SecretString composite accessors) is **paper design** relative to spike validation. Original Strategy §0 required spike evidence before Tech Spec progression; CP5 amendments landed post-spike-iter-2 without iter-3 closure. Gate 3 narrow-scope spike addresses this. | process | proposed | Cross-ref `gate-spike-iter3-dyn-safety`. Source: user Round 5 review recommendation («spike iter-3 narrow-scope BEFORE П1 trigger fires»). |
