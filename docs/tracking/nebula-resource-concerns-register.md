...
| R-013 | Transport topology has 0 Manager-level integration tests | 🟠 | post-cascade | Phase 1 §2.4 tech-lead | **landed post-cascade** ([PR #621](https://github.com/vanyastaff/nebula/pull/621)); 9 new `#[tokio::test]`s in `crates/resource/tests/transport_integration.rs` exercising `register_transport` / `register_transport_with` + `acquire_transport_default` + multiplexing + session-limit backpressure + recovery-gate admission + resilience profile + key isolation + `remove` + `graceful_shutdown` drain + custom scope. Mock transport (`MockTransport*` types via `mock_transport_type!` macro) tracks create / open_session / close_session counters; no real network deps. | post-cascade |
...
- 2026-04-28 — R-013 (post-cascade) landed via [PR #621](https://github.com/vanyastaff/nebula/pull/621): 9 Manager-level Transport integration tests added in `crates/resource/tests/transport_integration.rs`. Coverage: `register_transport` / `register_transport_with`, `acquire_transport_default`, multiplexing (one transport / many sessions), session-limit backpressure, `RecoveryGate` admission, `AcquireResilience` happy path, key isolation, `remove`, `graceful_shutdown` drain, custom scope.
