...
| nebula-schema        | stable for the internal `ValidSchema`/proof pipeline, structured `#[property(display/input/validate)]` derive primitive, `$root` rejection, boxed `ValidationError`, and versioned JSON Schema extension registry; remaining Phase-5 targets (`schema_type`, directional codec evidence, validator Condition v2, slot grammar, options-provider admission, trigger/deferred output gates) stay unshipped until their owners land code and tests | stable  | stable ([crates/schema/README.md](../crates/schema/README.md), [crates/schema/docs/DESIGN.md](../crates/schema/docs/DESIGN.md), [crates/schema/docs/JSON_SCHEMA_EXTENSIONS.md](../crates/schema/docs/JSON_SCHEMA_EXTENSIONS.md)) | stable for current schema consumers; future leaf-slot/output wiring remains tracked by integration follow-ups | n/a |
...
Last targeted revision: 2026-09-15 — **schema property primitive and export freeze.**
...
freezes the JSON Schema `x-nebula-*` extension key set in docs plus snapshots.
The stable row covers that implemented proof/export contract only; directional
codec evidence, checked Condition v2, slot grammar, options-provider admission
and trigger/deferred output gates remain target contracts.
