...
| nebula-api           | frontier | stable  | stable | partial (pure library; `apps/server` is its HTTP composition root. Start/Cancel producers and all five `EngineControlDispatch` methods exist, but current first-party roots do not install `ControlConsumer`. Knife steps 3+5 manually share in-memory ports and are component-integration evidence, not a deployed server+worker e2e proof. Defaults are intentionally separate today: server execution backend = process-local Memory when `API_EXECUTION_BACKEND` is unset; worker backend = its own `nebula-worker.db` SQLite file. Per-domain module structure under `domain/` (§12.7); Phases 1–4 stub completion still pending) | partial |
...
| nebula-plugin        | stable mutable registry path; partial default-public frozen plugin-set/flavor epoch and pure Graph-v1 compiler | stable  | stable | stable mutable path; pure authority-free compilation plus checked plan/flavor recorded forms and registry compatibility exist, but the frozen epoch still has zero production consumer until SQL retention, admission, persisted routing, and exact-flavor dispatch adopt it end to end | n/a |
| nebula-resilience    | stable   | stable  | stable | n/a | n/a |
| nebula-storage-port  | stable existing row-model seam; partial exact plan/flavor catalog | stable  | stable | partial for exact revisions: read/admin roles and opaque records exist, and paired migration 0041 supplies dormant SQLite/PostgreSQL layout constraints; no public reference mutation, SQL adapter, or production reference transaction exists until Task 13B; stable for existing ADR-0072 consumers | n/a |
...
| nebula-storage       | partial  | partial (SQLite/internal reference suites are local evidence; live PostgreSQL remains a release gate) | stable | stable existing paths; Task 13A exact revision lifecycle/reference model plus dormant paired 0041 SQL layout/constraints are component evidence only, with SQL adapters and production owner-transaction mutation absent | partial |
