...
| nebula-storage-port  | stable existing row-model seam; partial exact plan/flavor catalog | stable  | stable | partial for Task 13A exact revisions: read/admin roles and opaque records exist, but no public reference mutation and no SQLite/PostgreSQL implementation until Tasks 14A/13B; stable for existing ADR-0072 consumers | n/a |
...
| nebula-storage       | partial  | partial (SQLite/internal reference suites are local evidence; live PostgreSQL remains a release gate) | stable | stable existing paths; Task 13A exact revision lifecycle/reference model is InMemory component evidence only, with SQL and production owner-transaction mutation absent | partial |
...
compilation, and Task 13A exact-revision component boundary.** Removed the former opt-in feature
...
exact catalog roles, an InMemory drain/reference model, and engine install/load bridges. Existing
state-machine and mutable-registry paths remain stable. SQL durability, production owner-transaction
reference mutation, admission, bundle materialization, persisted routing, and exact-flavor dispatch
remain explicitly `partial`; Task 13A is not a production-consumption claim.
