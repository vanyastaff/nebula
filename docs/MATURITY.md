...
|---|---|---|---|---|---|
| nebula-action        | frontier | stable  | stable | partial (webhook sig `Required` by default at trait surface — ADR-0022; unsupported action checkpoint selection removed, historical non-default tags cannot readmit or project; retry intentionally has two layers: integration-internal resilience and operator-declared engine `retry_policy` after typed retryable failures. `ActionResult` grants no retry authority; stateful iteration durability remains a gap.) | n/a |
...
| nebula-schema        | frontier: policy-v2 property hardening is implemented, but full authoring, directional codecs, conditions, admitted file/blob references and domain-kind coverage remain incomplete; #995 is not yet a complete stable freeze | partial: focused [policy](../crates/schema/tests/schema_policy_epoch.rs), [unknown-kind](../crates/schema/tests/unknown_property_admission.rs), [derive](../crates/schema/tests/property_derive_contract.rs) and [size-cap](../crates/schema/tests/validation_error_size.rs) contracts | stable ([coverage and limits](../crates/schema/README.md#input-coverage), [extension registry](../crates/schema/docs/JSON_SCHEMA_EXTENSIONS.md)) | partial: existing proof-carrying consumers work; remaining input-family and leaf-slot contracts require end-to-end evidence | n/a |
| nebula-sdk           | partial (the curated one-dependency authoring surface covers `ActionMetadataDraft`, typed `Action::Input`/`Output`, `simple_action!`, `WorkflowBuilder`, credential `TestResult`, resource providers/custom topologies, and representative Action/Credential/Plugin/Resource/Schema/Validator derives; client and embedded remain gaps) | partial (SDK-only compile-pass fixtures exercise the supported authoring and derive paths, a renamed-leaf fixture checks derive path resolution, and 46 missing-name plus one opaque-bridge compile-fail probes pin internal metadata/proof/runtime authority out of both persona and `__private` paths) | stable (gaps, the curated perimeter, and unsupported direct-leaf workarounds documented) | n/a | n/a |
| nebula-storage       | partial  | stable for runtime authority (reference, SQLite, and required live PostgreSQL conformance); broader storage coverage remains partial | stable | partial (SQLite and PostgreSQL implement exact revision catalogs, live-reference admission, keyed start acceptance, atomic turn handoff, bounded durable checkpoints, operation ledger, and accepted-turn recovery. InMemory remains an internal reference and conformance adapter.) | partial |
| nebula-validator     | frontier | stable  | stable | stable | n/a |
| nebula-worker        | partial  | stable  | partial | partial (reusable exact-flavor control consumption and accepted-turn recovery are composed by `apps/worker`; deployment backend and lifecycle remain application-owned) | partial |
...
- **K3 (debt):** make the controller plus semantic idempotency/operation ledger the sole management
  writer; add the durable sentinel-to-reauth command; replace trace-only audit with transactional
...
Last targeted revision: 2026-09-16 — **credential-subsystem follow-up truth pass.** The
`nebula-credential` row no longer sends a reader to a document that records none of what it is
cited for: the ADR-0088 D1/D2/D6 residue is now written into `crates/credential/docs/DESIGN.md`
under "Remaining design work", where ADR-0088's own text already points. The row no longer names an
unresolvable crate-internal `CredentialRuntime`; the deployment credential-runtime composition
module is `apps/server/src/credential_composition.rs`, and the row's `resolve_for_slot` clause no
longer claims the execution seam. No crate state changed. The pass also corrected the propagated
copies of those claims, which had reached `crates/engine/docs/DESIGN.md`,
`crates/resource/docs/DESIGN.md`, and `crates/resource/src/manager/mod.rs`, so a reader meets the
same facts wherever the credential boundary is described.
Prior targeted revision: 2026-09-15 — **schema property admission hardening.**
`nebula-schema` now accepts structured `#[property(display(...), input(...),
validate(...))]` in `#[derive(Schema)]`, keeps presentation annotations as
non-authoritative projections, rejects removed `$root.foo` rule references with
a JSON Pointer rewrite, pins `ValidationError` to a small boxed shape, and
freezes the JSON Schema `x-nebula-*` extension keys and values in docs plus snapshots.
Policy v2 makes requiredness independent of presentation and distinguishes
historical schema evidence from fresh authority. Unsupported kinds cannot grant
value proof or export as unconstrained JSON Schema. The API remains `frontier`:
directional codecs, full property-family coverage, checked conditions, slots,
options-provider admission and trigger/deferred output gates still need evidence.
...
repository has no packaged multi-process deployment profile or where broader crate capabilities
remain incomplete; InMemory is described only as the internal reference adapter.
Last targeted revision: 2026-07-27 — **immutable execution-contract vocabulary, pure plan
compilation, and Tasks 13A/14A exact-revision component boundary.** Removed the former opt-in feature
names and made revision, frozen-registry, and Graph-v1 bundle primitives default-public. Added
canonical bundle identity/fingerprint vocabulary, structural wire validation, frozen component
snapshots, the authority-free Graph-v1 compiler, checked plan/flavor recorded forms, technical
exact catalog roles, an InMemory drain/reference model, engine install/load bridges, and dormant
paired 0041 SQLite/PostgreSQL layout constraints. Existing state-machine and mutable-registry paths
remain stable. SQL adapters, production owner-transaction reference mutation, admission, bundle
materialization, persisted routing, and exact-flavor dispatch remain explicitly `partial`; the
component and DDL evidence is not a production-consumption claim.
