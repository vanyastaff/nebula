...
| nebula-engine        | partial  | stable  | stable | partial (the first-party worker constructs the exact-flavor `ControlConsumer`, atomically accepts claimed Start/Resume/Restart turns, recovers accepted turns, and drives remote effects through the durable operation ledger. Execution state carries exact plan, flavor, and bundle pins; genuine resume never reloads a newer workflow. Runtime authority is covered across the reference adapter, SQLite, and required live PostgreSQL conformance. Broader engine maturity remains partial because result-driven `ActionResult::Retry` is not public and the credential bridge remains test-harness only.) | n/a |
