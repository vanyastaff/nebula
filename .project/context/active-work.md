...
- **Sandbox subsystem** (2026-04-13): Phase 0 landed — stateless `CapabilityGated`/`Isolated` now dispatch through `self.sandbox`, ignored test un-ignored (3177 workspace tests green). Phase 1 pending (gRPC broker + `PluginSupervisor` + long-lived per-credential-scope lifecycle + Reattach). Full roadmap: `docs/plans/2026-04-13-sandbox-roadmap.md`, research in `.project/context/research/sandbox-prior-art.md`. Permission manifest model explicitly **deferred** — see roadmap §D4.
