...
- **PollResult.** `poll(&mut cursor) -> PollResult { events, override_next }`. `From<Vec<E>>` for ergonomics. `override_next: Option<Duration>` for per-cycle interval override (e.g., Retry-After). Cursor is `&mut` — adapter clones before poll for `RetryBatch` rollback.
