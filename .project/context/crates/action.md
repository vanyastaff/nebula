...
- **PollCursor<C>.** Wraps cursor with checkpoint capability. `Deref/DerefMut` to `C` for transparent access. `checkpoint()` saves progress during paginated polls. Adapter rollback strategy: full success → current position; partial_error → last checkpoint; Err → pre-poll. Non-paginated actions ignore checkpoints entirely.
- **PollResult.** `poll(&mut PollCursor<C>) -> PollResult { events, override_next, partial_error }`. `From<Vec<E>>` for ergonomics. `partial()` constructor for paginated failures mid-way: events dispatched, cursor rolls back to checkpoint.
