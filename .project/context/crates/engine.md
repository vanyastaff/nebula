...
- **Proactive credential refresh failure is a typed error, not a WARN log** (#306, Batch 5D). When the `credential_refresh` hook returns `Err`, `NodeTask::run` constructs `ActionError::CredentialRefreshFailed { action_key, source }` and returns it via `EngineError::Action`, which routes through `handle_node_failure` so `ErrorStrategy` decides retry/dead-letter/fail. The action body is **never** invoked when refresh fails — the node transitions directly to Failed with the typed cause in `node_errors`. The refresh await is also wrapped in `tokio::select!` against `cancel.cancelled()` so a dying credential store cannot block shutdown. Default classification of `CredentialRefreshFailed` is retryable (transient store outage assumption); operators wanting fail-fast can override per-action.
